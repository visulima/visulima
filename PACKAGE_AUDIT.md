...
- 🟡 `pail`: "warn" vs RFC5424 "warning" mismatch sends warning/critical/alert/emergency to stdout instead of stderr. _(Partial: stream/console routing fixed, but `src/reporter/json/json-reporter.server.ts:72` still uses the `["error","warn"]` name.)_
...
| 🟡     | `@visulima/pail`                     | Fix the "warn" vs RFC5424 "warning" level-name mismatch in stream/console routing so warning, critical, alert, and emergency logs go to stderr (and console.warn in browsers) instead of stdout. _(Stream/console routing fixed; json-reporter.server.ts:72 still mismatched.)_                                                                                     |
