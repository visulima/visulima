// Mirrors the @visulima/tui config: these components were extracted from that
// package and share its idioms (canvas index arithmetic, ANSI template literals,
// prop spreading), so the same rule relaxations apply.
...
export default createConfig(
    ...
    {
        files: ["src/**/*.ts", "src/**/*.tsx", "__tests__/**/*.ts", "__tests__/**/*.tsx", "examples/**/*.tsx"],
        rules: {
            // Every component and hook in @visulima/tui deliberately ships both a
            // default and an identically-named named export (`export default Box;
            // export { Box }`), so `import Box from ".../components/box"` is the
            // intended usage rather than a mistaken default import. The rule can't
            // tell the two apart and fires on every such import.
            "import/no-named-as-default": "off",
        },
    },
    {
        // Highlighter-backed components. @visulima/tui's emitted
        // dist/ink/public/highlighter.d.ts imports ThemedToken/TokensResult from
        // @shikijs/core and HighlighterGeneric from @shikijs/types — packages tui
        // does not declare (they are transitive under shiki), so under pnpm's
        // strict layout they do not resolve from here and every value crossing
        // that boundary is seen as an error type. Nothing is actually untyped:
        // fix belongs in tui's dts emit (declare them, or re-export the types
        // from tui itself), after which these can come back on.
        files: ["src/code.tsx", "src/diff-view.tsx"],
        rules: {
    ...
    {
        // Pre-existing findings in components lifted verbatim out of @visulima/tui.
        // They never surfaced there because lint-staged only lints changed files
        // and these had not changed; the move makes all 90 "new" at once. Left as
        // findings to address on their own rather than silently rewritten inside a
        // migration whose point is that the code is unchanged.
        files: ["src/animate-presence.tsx"],
        rules: {
            "react-you-might-not-need-an-effect/no-adjust-state-on-prop-change": "off",
        },
    },
    {
        // See above — transition.tsx already carries an inline disable for the
        // adjust-state rule, so only the event-handler one is needed here.
        files: ["src/select-input.tsx", "src/transition.tsx"],
        rules: {
            "react-you-might-not-need-an-effect/no-event-handler": "off",
        },
    },
    {
        files: ["src/unordered-list.tsx"],
        rules: {
            "@typescript-eslint/no-unsafe-return": "off",
        },
    },
    {
        // Same story as above, for tests moved verbatim from @visulima/tui.
        files: ["__tests__/ink/form-controls.test.tsx", "__tests__/ink/timed-components.test.tsx"],
        rules: {
