...
const randomSeries = (length: number, max = 100): ReadonlyArray<number> =>
...
const App = () => {
    ...
    const scatter = React.useMemo(
        () => Array.from({ length: 40 }, () => ({ x: Math.random() * 100, y: Math.random() * 100 })),
        [version],
    );

    const heatmapData = React.useMemo(
        () => Array.from({ length: 6 }, () => Array.from({ length: 12 }, () => Math.round(Math.random() * 100))),
        [version],
    );

    return (
        <Box flexDirection="column" gap={1} padding={1}>
            <Text bold color="cyan">Sparkline</Text>
            ...
            <Box gap={4}>
                ...
                <AreaChart
                    fillDensity="medium"
                    height={6}
                    series={[{ data: trend, label: "Load" }]}
                    showLegend={false}
                    width={32}
                />
            </Box>
            <Text bold color="cyan">ScatterPlot + Heatmap</Text>
            <Box gap={4}>
                <ScatterPlot
                    height={6}
                    maxX={100}
                    ...
                    minX={0}
                    minY={0}
                    series={[{ data: scatter, label: "points" }]}
                    showLegend={false}
                    width={24}
                />
                <Heatmap data={heatmapData} rowLabels={["A", "B", "C", "D", "E", "F"]} />
            </Box>
            <Text bold color="cyan">Gauge (+/- to adjust)</Text>
            <Gauge
                label="CPU load"
                showLegend
                thresholds={[
