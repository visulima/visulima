...
mod tests {
  ...
  fn test_first_frame_diffs_all_cells() {
    ...
    assert!(
      diff.len() > "\x1b[0m".len(),
      "First frame should produce output for all cells"
    );
  }
  ...
  fn test_second_identical_call_produces_empty_diff() {
    ...
    assert_eq!(
      diff2, "\x1b[0m",
      "Second call should only contain the reset prefix"
    );
  }
  ...
  fn test_contiguous_cells_skip_cursor_move() {
    ...
    assert_eq!(
      move_count, 1,
      "Only one cursor move expected for contiguous cells"
    );
    assert!(
      diff.contains("AB") || diff.ends_with("AB"),
  ...
  fn test_non_contiguous_cells_emit_cursor_move() {
    ...
    assert_eq!(
      move_count, 2,
      "Two cursor moves expected for non-contiguous cells"
    );
  }
  ...
  fn test_wide_glyph_advances_cursor_by_display_width() {
    ...
    assert!(
      diff.contains("界B"),
      "Continuation cells must not emit a printable spacer"
    );
    ...
    assert_eq!(
      move_count, 1,
      "Wide glyph should keep cursor progression contiguous"
    );
  }
  ...
  fn test_continuation_only_update_is_non_printing() {
    ...
    assert_eq!(
      diff, "\x1b[0m",
      "Continuation-only updates should not emit printable output"
    );
  }
  ...
  fn test_style_reset_forces_color_redraw() {
    ...
    assert!(
      rest.contains("\x1b[0m"),
      "Should contain style reset after A"
    );
    assert!(
      rest.contains("\x1b[38;5;1m"),
      "Should re-emit fg after style reset"
    );
    assert!(
      rest.contains("\x1b[48;5;2m"),
      "Should re-emit bg after style reset"
    );
  }
  ...
  fn test_color_diff_only_emits_changed_color() {
    ...
    assert_eq!(
      fg1_count, 1,
      "fg=1 should only be emitted once since it doesn't change"
    );
  }
  ...
  fn test_row_offset_shifts_cursor_position() {
    ...
    assert!(
      diff.contains("\x1b[6;1H"),
      "Row offset should shift cursor to row 6"
    );
  }
  ...
  fn test_resize_resets_front_buffer() {
    ...
    assert!(
      diff.len() > "\x1b[0m".len(),
      "Resize should reset front buffer"
    );
  }
}
