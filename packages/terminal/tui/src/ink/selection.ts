 ...
 *
 * Provides browser-like Selection/Range classes that operate on the Ink DOM tree,
 * enabling text selection highlighting and copy-to-clipboard functionality.
 *
...
export const comparePoints = (nodeA: DOMNode, offsetA: number, nodeB: DOMNode, offsetB: number): number => {
    ...
    while (index < pathA.length && index < pathB.length && pathA[index] === pathB[index]) {
        lca = pathA[index];
        index++;
    }
    ...
    if (index === pathA.length) {
        const childB = pathB[index];
        const indexChildB = (nodeA as DOMElement).childNodes.indexOf(childB!);

        if (offsetA <= indexChildB) {
            return -1;
        }
    ...
    if (index === pathB.length) {
        const childA = pathA[index];
        const indexChildA = (nodeB as DOMElement).childNodes.indexOf(childA!);

        if (indexChildA < offsetB) {
            return -1;
        }
...
export class Range {
    ...
    private updateCommonAncestor(): void {
        ...
        const startPath = getPathToRoot(this.startContainer);
        const endPath = getPathToRoot(this.endContainer);

        let common: DOMNode | undefined;

        for (let index = 0; index < Math.min(startPath.length, endPath.length); index++) {
            if (startPath[index] === endPath[index]) {
                common = startPath[index]!;
            } else {
                break;
            }
        }
