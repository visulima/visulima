...
describe("ProgressBar", () => {
    it("should render a full bar at 100%", () => {
        const output = renderToString(<ProgressBar percent={1} columns={10} />);

        expect(output).toBe("\u2588".repeat(10));
    });

    it("should render a half bar at 50%", () => {
        const output = renderToString(<ProgressBar percent={0.5} columns={10} />);

        expect(output).toBe("\u2588".repeat(5));
    });

    it("should render an empty bar at 0%", () => {
        const output = renderToString(<ProgressBar percent={0} columns={10} />);

        expect(output).toBe("");
    });

    it("should respect left margin", () => {
        const output = renderToString(<ProgressBar percent={1} columns={20} left={5} />);
    ...
    it("should respect right margin", () => {
        const output = renderToString(<ProgressBar percent={1} columns={20} right={5} />);
    ...
    it("should respect both left and right margins", () => {
        const output = renderToString(<ProgressBar percent={1} columns={20} left={3} right={7} />);
    ...
    it("should use a custom character", () => {
        const output = renderToString(<ProgressBar percent={1} columns={5} character="=" />);

        expect(output).toBe("=====");
    });

    it("should pad with spaces when rightPad is true", () => {
        const output = renderToString(<ProgressBar percent={0.5} columns={10} rightPad />);
    ...
    it("should not pad when rightPad is false", () => {
        const output = renderToString(<ProgressBar percent={0.5} columns={10} rightPad={false} />);

        expect(output).toBe("\u2588".repeat(5));
    });

    it("should clamp percent above 1 to 1", () => {
        const output = renderToString(<ProgressBar percent={1.5} columns={10} />);
    ...
    it("should handle margins exceeding columns gracefully", () => {
        const output = renderToString(<ProgressBar percent={1} columns={10} left={6} right={6} />);
    ...
    it("should use terminal width when columns is 0", () => {
        ...
        const output = renderToString(<ProgressBar percent={0.1} columns={0} />);
    ...
    it("should floor partial fill values", () => {
        ...
        const output = renderToString(<ProgressBar percent={0.33} columns={10} />);

        expect(output).toBe("\u2588".repeat(3));
    });

    it("should render rightPad correctly with full bar", () => {
        const output = renderToString(<ProgressBar percent={1} columns={10} rightPad />);
