...
describe("hooks-use-input-ime", () => {
    ...
    it.skipIf(!ptyAvailable)("useInput - buffers Vietnamese IME input", async () => {
        expect.hasAssertions();

        const ps = term("use-input-ime", ["vietnamese"]);
    ...
    // On Linux, node-pty may deliver separate ps.write() calls as a single chunk,
    // causing both Unicode characters to arrive in one input event. The fixture
    // expects them individually and never exits. macOS PTY handles this correctly.
    it.skipIf(!ptyAvailable || process.platform === "linux")("useInput - passes IME characters individually when imeEnabled is false", async () => {
        expect.hasAssertions();

        const ps = term("use-input-ime", ["imeDisabled"]);
