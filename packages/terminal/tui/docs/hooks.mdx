...
For a visual component, see [`<Timer>`](/docs/packages/tui-kit/components#timer).
...
For a visual component, see [`<Stopwatch>`](/docs/packages/tui-kit/components#stopwatch).

## `useKeyBindings`

Declarative keybinding hook. Registers multiple key bindings with handlers and returns the enabled bindings for display in a [`<Help>`](/docs/packages/tui-kit/components#help) component.
...
```tsx
import { useTreeViewState } from "@visulima/tui-kit/tree-view";
