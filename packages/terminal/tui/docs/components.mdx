...
```tsx
<BarChart data={[{ value: 12, label: "Mon" }, { value: 8, label: "Tue" }]} />
```
...
```tsx
<Button onPress={() => save()} autoFocus>Save</Button>
```
...
<Canvas
    ...
    height={5}
    version={frame}
    draw={(ctx) => ctx.setCell(0, 0, "█", { color: "cyan" })}
/>
```
...
<StaticRender deps={[completed.length]}>
    {() => (
        <Box flexDirection="column">
            {completed.map((task) => (
                <Text key={task.id} color="green">✓ {task.name}</Text>
            ))}
        </Box>
    )}
</StaticRender>
