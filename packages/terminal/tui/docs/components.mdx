---
title: Components
description: "Reference for all React components in @visulima/tui: Alert, Badge, Box, Code, ConsoleOverlay, FilePicker, Text, BigText, ConfirmInput, DiffView, Gradient, Help, Link, Markdown, MultiSelect, Newline, OrderedList, Paginator, Slider, Spacer, Static, StatusMessage, Stopwatch, Tab, Tabs, Textarea, TextInput, Timer, Transform, TreeView, Spinner, ProgressBar, SelectInput, Table, UnorderedList"
---
...
import { Gradient } from "@visulima/tui/components/gradient";
...
import { Stopwatch } from "@visulima/tui/components/stopwatch";
...
`BigText`, `Code`, `DiffView`, `Markdown` and `Table` each ship with at least one heavy optional peer dependency. They are exported from per-component subpaths so the optional deps are only loaded when you actually import the component.

```tsx
import { BigText } from "@visulima/tui/components/big-text"; // requires: cfonts
import { Code } from "@visulima/tui/components/code"; // requires: shiki, @shikijs/langs, @shikijs/themes
import { DiffView } from "@visulima/tui/components/diff-view"; // requires: diff (+ shiki for syntax highlighting)
import { Markdown } from "@visulima/tui/components/markdown"; // requires: marked (+ shiki for code blocks)
import { Table } from "@visulima/tui/components/table"; // requires: @visulima/tabular
```

See the [Package Map › Heavy component subpaths](./package-map.mdx#heavy-component-subpaths) table for a full peer-dep matrix and `npm install` commands.
...
> **Subpath import.** Imported from `@visulima/tui/components/code`. Requires the optional peers `shiki`, `@shikijs/langs`, `@shikijs/themes`.
>
> ```bash npm2yarn
...
> **Subpath import.** Imported from `@visulima/tui/components/diff-view`. Requires the optional peer `diff`. To enable syntax highlighting via the `language` prop, also install `shiki`, `@shikijs/langs`, and `@shikijs/themes`.
>
> ```bash npm2yarn
> npm install diff
...
> **Subpath import.** Imported from `@visulima/tui/components/big-text`. Requires the optional peer `cfonts`.
>
> ```bash npm2yarn
> npm install cfonts
> ```
...
```tsx
import { Link } from "@visulima/tui/components/link";
import { Text } from "@visulima/tui/components/text";
...
> **Subpath import.** Imported from `@visulima/tui/components/markdown`. Requires the optional peer `marked`. Code blocks pull in `shiki`, `@shikijs/langs`, `@shikijs/themes`; tables pull in `@visulima/tabular` — install whichever your content needs.
>
> ```bash npm2yarn
> npm install marked
...
For headless usage, use the [`useStopwatch`](./hooks#usestopwatch) hook directly.
...
For headless usage, use the [`useTimer`](./hooks#usetimer) hook directly.
...
```tsx
import { Box } from "@visulima/tui/components/box";
import { ConsoleOverlay } from "@visulima/tui/components/console-overlay";
...
```tsx
import type { SelectInputEntry, SelectInputSeparator } from "@visulima/tui/components/select-input";
...
> **Subpath import.** Imported from `@visulima/tui/components/table`. Requires the optional peer `@visulima/tabular`.
>
> ```bash npm2yarn
> npm install @visulima/tabular
> ```
...
```tsx
import { treeViewTheme } from "@visulima/tui/components/tree-view";
