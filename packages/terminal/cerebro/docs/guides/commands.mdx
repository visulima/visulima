...
Use `loader` instead of `execute` to defer importing the handler module
until the command is actually invoked. Help, completion, and option
validation work from the metadata alone — they never trigger the loader.
See the [advanced guide](/guides/advanced#lazy-command-loading) for the
full pattern.
...
cli.addCommand({
    ...
    loader: () => import("./commands/build.handler"),
});
...
A command must declare either `execute` or `loader`, but not both.
