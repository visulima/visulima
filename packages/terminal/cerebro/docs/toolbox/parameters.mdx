...
Declare `arguments` instead of `argument` to read each positional by name from
`toolbox.args`:
...
cli.addCommand({
    ...
    execute: ({ args, argument }) => {
        ...
        args.source; // "a.txt"
        args.targets; // ["b/", "c/"]
        argument; // ["a.txt", "b/", "c/"] — still the raw list
    },
});
```

Slot order follows array order, names are camelCased, and only the last entry may
be `multiple`. See [Named Positional Arguments](/docs/cerebro/guides/options#named-positional-arguments).
...
The shortest path is `defineCommand`, which infers `options`, `env` and `args`
from the definitions — no second declaration to keep in sync:
...
You can still annotate the toolbox by hand if you prefer:
