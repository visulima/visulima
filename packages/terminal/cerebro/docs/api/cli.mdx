...
Add a global option available to all commands. Global options are parsed alongside
command-specific options and displayed in the help output under "Global Options".

```typescript
cli.addGlobalOption(option: OptionDefinition): this
```
...
Throws an error if the option name or alias conflicts with a built-in global option
(`verbose`, `debug`, `help`, `quiet`, `version`, `no-color`, `color`) or a previously
added custom global option.
...
Get all global options (built-in + custom):

```typescript
cli.getGlobalOptions(): OptionDefinition[]
```
