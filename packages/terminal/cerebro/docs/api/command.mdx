...
interface Command<O extends OptionDefinition = OptionDefinition, TContext extends Toolbox = Toolbox> {
    ...
    /** Named positionals, in slot order. */
    arguments?: ArgumentDefinition[];
    options?: OptionDefinition[] | Record<string, Omit<OptionDefinition, "name">>;
    env?: EnvDefinition[] | Record<string, Omit<EnvDefinition, "name">>;
    hidden?: boolean;
    group?: string;
...
Use this for commands whose handler pulls in heavy dependencies — the
import cost is paid only when the user actually invokes the command.
...
Named positional arguments, surfaced on `toolbox.args`. Array order is slot order;
only the last entry may set `multiple: true`. Mutually exclusive with `argument`.

An array rather than a record because slot order is load-bearing — an alphabetical
...
```typescript
{
    arguments: [
        ...
        { name: "targets", type: String, multiple: true, required: true },
    ];
}
```
...
**Type:** `OptionDefinition[] | Record<string, Omit<OptionDefinition, "name">>`  
**Required:** No

Command options. The record form is keyed by option name, so `name` is omitted:
...
**Type:** `EnvDefinition[] | Record<string, Omit<EnvDefinition, "name">>`  
**Required:** No
...
- Converted to camelCase (`API_KEY` → `apiKey`)
