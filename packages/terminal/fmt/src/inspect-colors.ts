...
//
// Lazily constructed: this 148-entry map is only needed when a `%c` substitution references a named
// CSS color, which never happens for the common "plain log formatting" case. Building it on first
// `parseCssColor` call instead of at module load shrinks startup cost for every consumer (pail et al.).
let colorKeywordsCache: Map<string, string> | undefined;

const getColorKeywords = (): Map<string, string> => {
    if (colorKeywordsCache !== undefined) {
        return colorKeywordsCache;
    }

    colorKeywordsCache = new Map<string, string>([
        ["aliceblue", "#f0f8ff"],
        ["antiquewhite", "#faebd7"],
        ["aqua", "#00ffff"],
        ["aquamarine", "#7fffd4"],
...
const parseCssColor = (colorString: string): [number, number, number] | null => {
    const colorKeywords = getColorKeywords();
