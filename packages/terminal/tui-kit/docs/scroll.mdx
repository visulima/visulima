...
1. **CSS-level scrolling** via `Box` props (`overflow="scroll"`, `scrollTop`, `sticky`) — integrated into the layout engine with native scrollbar rendering and sticky header support. See the [Box component docs](/docs/packages/tui/components#overflow-and-css-level-scrolling).
...
```tsx
import { ControlledScrollView } from "@visulima/tui-kit/controlled-scroll-view";
import { ScrollBar } from "@visulima/tui-kit/scroll-bar";
import { ScrollBarBox } from "@visulima/tui-kit/scroll-bar-box";
import { ScrollList } from "@visulima/tui-kit/scroll-list";
