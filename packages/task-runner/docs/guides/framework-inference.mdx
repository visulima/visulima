...
| Remix | `@remix-run/react` | `REMIX_PUBLIC_*` |
| Astro | `astro` | `PUBLIC_*` |
| SvelteKit | `@sveltejs/kit` | `PUBLIC_*` |
| Solid Start | `@solidjs/start` | `VITE_*` |
...
```typescript
import { detectFrameworks, getFrameworkEnvVars } from "@visulima/task-runner";

const frameworks = await detectFrameworks("/path/to/package.json");
// [{ name: "Next.js", envPrefixes: ["NEXT_PUBLIC_"] }]

const envVars = await getFrameworkEnvVars("/path/to/package.json");
