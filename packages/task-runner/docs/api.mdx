...
### `createTaskGraph(tasks, projectGraph, config)`

Creates a dependency-ordered task graph from task definitions.

### `getTaskId(project, target, configuration?)`

Creates a task ID string (`"project:target:configuration"`).
...
- `findCycle(graph)` - Returns first cycle found, or null
...
const hasher = new IncrementalFileHasher({
    workspaceRoot,
    snapshotPath: ".task-runner-cache/snapshot.json",
});
...
- `retrieve(hash, cacheDir)` - Download from remote
- `store(hash, cacheDir)` - Upload to remote
...
- `hashForPackage(packageJsonPath)` - Returns `PackageLockfileHash | null`
- `lockfileType` - Detected format (`"npm" | "pnpm" | "yarn" | null`)
...
- `getFrameworkEnvVars(packageJsonPath, env?)` - Returns matching env vars

## Affected Detection

- `getChangedFiles(options)` - Git diff to get changed file paths
- `getAffectedProjects(files, projectGraph)` - Map files to projects
- `filterAffectedTasks(tasks, affectedProjects)` - Filter tasks
