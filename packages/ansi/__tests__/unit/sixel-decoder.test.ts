...
describe("sixelDecoder", () => {
    ...
    it("should decode a simple Sixel image with raster, color, and data", () => {
        ...
        // Palette check
        expect(decoded.palette.colors[0]).toStrictEqual({ b: 0, g: 0, r: 255 }); // Red
        ...
        expect(decoded.palette.colors[1]).toStrictEqual(gray);
        ...
        expect(decoded.pixels).toStrictEqual(expectedPixels);
    });
    ...
    it("should handle Sixel string with only raster attributes", () => {
        ...
        expect(decoded.palette.colors.slice(0, SIXEL_STANDARD_PALETTE_SIZE)).toStrictEqual(initialPalette.colors.slice(0, SIXEL_STANDARD_PALETTE_SIZE));
    });
    ...
    it("should correctly decode the repeat operator (!)", () => {
        ...
        expect(decodedWithColor.pixels).toStrictEqual(finalExpected);
    });
    ...
    it("should correctly decode the carriage return ($)", () => {
        ...
        expect(decodedColor.palette.colors[1]).toStrictEqual({ b: 255, g: 0, r: 0 }); // Blue
        expect(decodedColor.palette.colors[2]).toStrictEqual({ b: 0, g: 255, r: 0 }); // Green
        ...
        expect(decodedColor.pixels).toStrictEqual(expectedPixelsColor);
    });
    ...
    it("should handle palette color definitions and selections near limits", () => {
        ...
        expect(decoded.palette.colors[MAX_PALETTE_INDEX]).toStrictEqual({ b: 77, g: 51, r: 26 }); // Scaled 10,20,30
        ...
        expect(decoded.palette.colors[1]).toStrictEqual({ b: 0, g: 0, r: 255 });
    ...
    describe("malformed Sixel Strings", () => {
        ...
        it("should handle malformed repeat string and attempt to recover", () => {
            ...
            expect(d.pixels).toStrictEqual(e);
        });
        ...
        it("should handle string ending abruptly in command", () => {
            ...
            expect(decoded.palette.colors[1]).toStrictEqual(DEFAULT_SIXEL_COLORS[1]); // Should be the default color for index 1
        ...
        it("should skip unexpected characters gracefully", () => {
            ...
            expect(decoded.pixels).toStrictEqual(expected);
        });
    });
});
