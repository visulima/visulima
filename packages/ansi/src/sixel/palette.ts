...
const createQuantizationCube = (colorEntries: { color: SixelColor; count: number }[]): QuantizationCube | null => {
 ...
 */
const medianCutQuantize = (imageData: RawImageData, maxColors: number): SixelColor[] => {
    if (maxColors <= 0) {
        return [];
    }
    ...
    for (let index = 0; index < imageData.data.length; index += 4) {
        const r = imageData.data[index] as number;
        const g = imageData.data[index + 1] as number;
        const b = imageData.data[index + 2] as number;
    ...
    while (pq.len() < maxColors && pq.len() > 0) {
        ...
        for (let index = 0; index < cubeToSplit.colors.length; index += 1) {
            combinedEntries.push({ color: cubeToSplit.colors[index] as SixelColor, count: cubeToSplit.pixelCounts[index] as number });
        }
    ...
    while (count < maxColors) {
        ...
        for (let index = 0; index < cube.colors.length; index += 1) {
            const color = cube.colors[index] as SixelColor;
            const pixelCount = cube.pixelCounts[index] as number;
...
export default medianCutQuantize;
