...
const tooltip = ({ message }: { message?: string } = {}): {
    ...
    return {
        html: message ? `<div class="custom-tooltip-container relative inline-block ml-2">
          ...
          <span id="causes-tooltip-content" class="opacity-0 invisible transition-opacity inline-block absolute z-10 py-1 px-2 bg-gray-900 text-xs font-medium text-white rounded shadow-sm dark:bg-slate-700 whitespace-normal" role="tooltip" style="left: 100%; top: 50%; transform: translateY(-50%) translateX(0.5rem); min-width: 200px;">
            ${message}
          </span>
        </div>` : "",
        script: `
