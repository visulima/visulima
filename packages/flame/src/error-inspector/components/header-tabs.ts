...
export const headerTabs = (tabs: HeaderTab[]): string => {
    ...
    ${tabs
        .map((t) => {
            const base =
                "relative px-3 py-1.5 rounded-[var(--flame-radius-md)] text-sm font-medium cursor-pointer text-[var(--flame-text-muted)] dark:text-[var(--flame-neutral)] transition-all duration-150 hover:text-[var(--flame-text)] hover:bg-[var(--flame-surface)] focus:outline-hidden focus:ring-2 ring-[var(--flame-red-orange)] inline-flex gap-2 items-center";
            const active =
