...
description: Advanced patterns and techniques for sophisticated error handling
---
...
<Tabs items={['Protocol Buffer', 'MessagePack', 'Custom Binary']}>
  <Tab value="Protocol Buffer">
    ```typescript title="protobuf-handler.ts"
  ...
  <Tab value="MessagePack">
    ```typescript title="msgpack-handler.ts"
    import httpHandler from "@visulima/error-handler/handler/http/node";
  ...
  <Tab value="Custom Binary">
    ```typescript title="binary-handler.ts"
    import httpHandler from "@visulima/error-handler/handler/http/node";
...
```typescript title="conditional-responses.ts"
const handler = await httpHandler(error, {
    extraHandlers: [
        {
...
```typescript title="handler-priority.ts"
const handlers = [
    {
...
```typescript title="quality-factor.ts"
...
```typescript title="language-negotiation.ts"
const handler = await httpHandler(error, {
    extraHandlers: [
        {
            regex: /application\/json/,
...
<Accordions>
  <Accordion title="Request Context">
    ```typescript title="request-context.ts"
    const handler = await httpHandler(error, {
  ...
  <Accordion title="User Context">
    ```typescript title="user-context.ts"
    const handler = await httpHandler(error, {
        onError: async (error, request, response) => {
...
<Tabs items={['Sentry', 'Custom Tracker', 'Aggregated']}>
  <Tab value="Sentry">
    ### Sentry Integration
  ...
  <Tab value="Custom Tracker">
    ### Custom Error Tracking

    ```typescript title="custom-tracker.ts"
    class ErrorTracker {
  ...
  <Tab value="Aggregated">
    ### Aggregated Error Reporting

    ```typescript title="aggregated-reporting.ts"
...
```typescript title="tenant-error-pages.ts"
const handler = await httpHandler(error, {
...
```typescript title="tenant-policies.ts"
const handler = await httpHandler(error, {
    onError: async (error, request, response) => {
        const tenant = await getTenantFromRequest(request);
...
<Tabs items={['Circuit Breaker', 'Retry with Backoff', 'Fallback']}>
  ...
  <Tab value="Retry with Backoff">
    ...
    ```typescript title="retry-backoff.ts"
  ...
  <Tab value="Fallback">
    ...
    const handler = await httpHandler(error, {
        onError: async (error, request, response) => {
            try {
                // Try primary logging service
                await primaryLogger.error({ error });
            } catch (primaryError) {
                try {
                    // Fallback to secondary service
                    await secondaryLogger.error({ error });
                } catch (secondaryError) {
                    // Last resort: console
                    console.error("All loggers failed:", error);
                }
            }
        },
    });
...
<Tabs items={['Unit Tests', 'Integration Tests', 'E2E Tests']}>
  <Tab value="Unit Tests">
    ...
    ```typescript title="handler.test.ts"
  ...
  <Tab value="E2E Tests">
    ...
    test("should not show stack trace in production", async ({ page }) => {
        await page.goto("/error");
...
<Accordions>
  <Accordion title="Lazy Loading Error Pages">
    ```typescript title="lazy-loading.ts"
  ...
  <Accordion title="Error Handler Caching">
    ```typescript title="handler-caching.ts"
  ...
  <Accordion title="Async Error Logging">
    ```typescript title="async-logging.ts"
...
<Callout type="tip">
  For high-traffic applications, consider implementing error sampling and async logging to minimize performance impact.
</Callout>
