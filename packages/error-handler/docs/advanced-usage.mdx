...
```ts tab="Protocol Buffer" title="protobuf-handler.ts"
...
```ts tab="MessagePack" title="msgpack-handler.ts"
import httpHandler from "@visulima/error-handler/handler/http/node";
...
```ts tab="Custom Binary" title="binary-handler.ts"
import httpHandler from "@visulima/error-handler/handler/http/node";
...
```ts title="conditional-responses.ts"
const handler = await httpHandler(error, {
    extraHandlers: [
        {
...
```ts title="language-negotiation.ts"
const handler = await httpHandler(error, {
    extraHandlers: [
        {
            regex: /application\/json/,
...
```ts tab="Sentry" title="sentry-integration.ts"
...
```ts tab="Custom Tracker" title="custom-tracker.ts"
class ErrorTracker {
...
```ts tab="Aggregated" title="aggregated-reporting.ts"
...
```ts title="tenant-error-pages.ts"
const handler = await httpHandler(error, {
...
```ts tab="Circuit Breaker" title="circuit-breaker.ts"
...
```ts tab="Retry with Backoff" title="retry-backoff.ts"
...
```ts tab="Fallback" title="fallback.ts"
const handler = await httpHandler(error, {
    onError: async (error, request, response) => {
        try {
...
```ts tab="Unit Tests" title="handler.test.ts"
...
```ts tab="Integration Tests" title="integration.test.ts"
...
```ts tab="E2E Tests" title="e2e.test.ts"
...
<details>
<summary>**Lazy Loading Error Pages**</summary>

```ts title="lazy-loading.ts"
...
<details>
<summary>**Error Handler Caching**</summary>

```ts title="handler-caching.ts"
...
<details>
<summary>**Async Error Logging**</summary>

```ts title="async-logging.ts"
