...
const solutions = async (
    ...
    return {
        html: `<div id="flare-solution-wrapper" class="relative w-full transition-all duration-300 ease-in-out overflow-visible">
    <button id="flare-solution-button" type="button" aria-label="Toggle solutions" aria-controls="flare-solution-content" aria-expanded="true" class="bg-[var(--flare-success-bg)] text-[var(--flare-text)] rounded-[var(--flare-radius-md)] p-2 absolute top-2 right-2 z-10 h-9 w-8 cursor-pointer shadow-[var(--flare-elevation-1)]">
        <span id="flare-solution-icon" class="dui" style="-webkit-mask-image: url('${closeIcon}'); mask-image: url('${closeIcon}')"></span>
    </button>
    <div id="flare-solution-content" role="region" aria-label="Suggested solutions" tabindex="-1" class="bg-[var(--flare-success-bg)] rounded-r-[var(--flare-radius-lg)] shadow-[var(--flare-elevation-2)] w-full h-full overflow-hidden transition-[max-height,opacity] duration-150 ease-in-out opacity-100">
        <div class="p-6 prose prose-sm prose-ul:list-none prose-hr:my-6 prose-hr:border-[var(--flare-success-border)] max-w-full relative text-[var(--flare-on-accent)]" aria-live="polite">
            ${await (async () => {
                if (!hint.header) {
                    return "<h2 class='text-[var(--flare-on-accent)]'>A possible solution to this error</h2>";
                }
        ...
        script: `(window.subscribeToDOMContentLoaded || function (fn) {
          ...
          // Get DOM elements
          const wrapper = document.querySelector('#flare-solution-wrapper');
          const button = document.querySelector('#flare-solution-button');
          const content = document.querySelector('#flare-solution-content');
          const icon = document.getElementById('flare-solution-icon');
