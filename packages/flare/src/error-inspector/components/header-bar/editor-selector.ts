...
const editorSelector = (editor?: Editors): string => {
  ...
  <select id=\"editor-selector\" class=\"peer py-2 px-3 pe-9 block w-56 bg-[var(--flare-surface)] border border-[var(--flare-border)] rounded-[var(--flare-radius-md)] text-sm text-[var(--flare-text)] shadow-[var(--flare-elevation-1)] hover:bg-[var(--flare-hover-overlay)] focus:outline-hidden focus:ring-1 focus:ring-[var(--flare-red-orange)]\">
    ${options}
  </select>
</div>`;
};
