...
describe("fetch RestFetch", () => {
    ...
    describe("default options", () => {
        it("should create RestFetch handler instance", () => {
            expect.assertions(1);

            expect(new RestFetch({ storage: new DiskStorage({ directory: "/files" }) })).toBeInstanceOf(RestFetch);
        });
    });
    ...
    describe("post", () => {
        ...
        it("should upload file with Content-Disposition header", async () => {
            ...
            const restHandler = new RestFetch({ logger, storage });
