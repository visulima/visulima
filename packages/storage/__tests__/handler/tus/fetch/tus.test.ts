...
describe("fetch Tus", () => {
    ...
    describe("default options", () => {
        it("should create TusFetch handler instance", () => {
            expect.assertions(1);

            expect(new TusFetch({ storage: new DiskStorage({ directory: "/files" }) })).toBeInstanceOf(TusFetch);
        });
    });
    ...
    describe("post", () => {
        it("should create upload resource and return 201 with location header", async () => {
            ...
            const tusHandler = new TusFetch({ storage });
