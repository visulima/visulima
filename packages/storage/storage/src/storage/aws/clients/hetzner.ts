...
const hetzner = (parameters?: CreateHetznerClientParameters): S3ClientConfig => {
    ...
    // for partial inputs.
    const accessKeyId = parameters?.accessKeyId ?? process.env.AWS_ACCESS_KEY_ID ?? process.env.HETZNER_ACCESS_KEY_ID ?? process.env.HETZNER_ACCESS_KEY;
    const location = parameters?.location ?? (process.env.HETZNER_LOCATION as CreateHetznerClientParameters["location"] | undefined);
    const secretAccessKey =
        parameters?.secretAccessKey ?? process.env.AWS_SECRET_ACCESS_KEY ?? process.env.HETZNER_SECRET_ACCESS_KEY ?? process.env.HETZNER_SECRET_KEY;
