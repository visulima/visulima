...
/**
 * Service for managing chunked upload operations.
 */
export class ChunkedUploadService {
     ...
     */
    public static isUploadComplete(chunks: ChunkInfo[], totalSize: number): boolean {
        if (chunks.length === 0) {
            return false;
        }
     ...
     */
    public static validateChunk(chunkOffset: number, chunkLength: number, totalSize: number, maxChunkSize?: number): void {
     ...
     */
    public static trackChunk(chunks: ChunkInfo[], chunkInfo: ChunkInfo): ChunkInfo[] {
     ...
     */
    public static calculateUploadProgress(
        chunks: ChunkInfo[],
        totalSize: number,
    ): {
        bytesWritten: number;
     ...
     */
    public static getBytesWritten<TFile extends UploadFile>(file: TFile): number {
        ...
        if (isChunkedUpload && Array.isArray(metadata._chunks)) {
            ...
            const progress = this.calculateUploadProgress(metadata._chunks as ChunkInfo[], totalSize);
     ...
     */
    public static isChunkedUpload<TFile extends UploadFile>(file: TFile): boolean {
        const metadata = file.metadata || {};
     ...
     */
    public static getTotalSize<TFile extends UploadFile>(file: TFile): number | undefined {
        const metadata = file.metadata || {};
        const isChunkedUpload = metadata._chunkedUpload === true;
