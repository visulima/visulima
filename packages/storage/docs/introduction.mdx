...
## Organizing Files into Folder Paths

You can organize uploaded files into folder structures by using a custom `filename` function that reads the folder path from request metadata. This works with all HTTP handlers (Multipart, REST, TUS) and all storage backends.

### Server-side Configuration

Configure your storage with a `filename` function that reads the folder path from metadata:
...
const storage = new DiskStorage({
    directory: "./uploads",
    filename: (file) => {
        // Read folder path from metadata
        const folderPath = file.metadata?.folderPath as string | undefined;

        if (folderPath) {
            // Return path with folder structure
            return `${folderPath}/${file.id}`;
        }

        // Default: just use file ID
        return file.id;
    },
});
...
You can send the folder path either as a form field (Multipart handler) or in the `x-file-metadata` header (REST handler):

#### Multipart Handler - Using Form Fields

Send the folder path directly in the form data:

```ts
const formData = new FormData();
formData.append("file", fileBlob);
formData.append("folderPath", "user-123/documents"); // Direct form field
...
Or send it as JSON in a `metadata` field:
...
#### Multipart Handler - Using Header (Alternative)

You can also use the `x-file-metadata` header:
...
For REST handler, use the `x-file-metadata` header:
...
TUS handler uses the `Upload-Metadata` header following the TUS protocol specification. The metadata must be base64-encoded:

```ts
// Encode folder path as base64
const folderPath = "user-123/documents";
const encodedPath = btoa(folderPath); // Base64 encode

const response = await fetch("/api/upload/tus", {
    method: "POST",
    headers: {
        "Upload-Length": file.size.toString(),
        "Upload-Metadata": `folderPath ${encodedPath}`, // TUS format: key base64value
    },
});

// For subsequent PATCH requests (uploading chunks)
...
const upload = new tus.Upload(file, {
    ...
    metadata: {
        folderPath: "user-123/documents", // Client library handles encoding
    },
    onSuccess: () => {
...
The file will be stored at: `./uploads/user-123/documents/{file.id}`
...
const storage = new DiskStorage({
    ...
    filename: (file) => {
        const folderPath = file.metadata?.folderPath as string | undefined;
        return folderPath ? `${folderPath}/${file.id}` : file.id;
    },
});
