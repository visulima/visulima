...
Hono is a fast, lightweight web framework for Cloudflare Workers, Deno, Bun, and other Web API environments.
...
Cloudflare Workers provide a serverless execution environment with Web API support.

```ts
import { DiskStorage, Multipart } from "@visulima/upload";

// Initialize once (outside the fetch handler for better performance)
const storage = new DiskStorage({ directory: "./uploads" });
const multipart = new Multipart({ storage });
...
```ts
import { DiskStorage, Multipart, ImageTransformer } from "@visulima/upload";
...
app.get("/images/:id", async (c) => {
    ...
    try {
        const result = await imageTransformer.transform(id, {
