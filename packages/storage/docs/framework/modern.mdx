...
Visulima Storage provides framework-specific import paths for convenience. All of these paths export the same handlers (`Multipart`, `Rest`, `Tus`) - they're aliases that make it clear which framework you're targeting:

- **`@visulima/storage/handler/http/fetch`** - Generic Web API (works everywhere)
- **`@visulima/storage/handler/http/hono`** - Hono framework (alias for fetch)
...
> **Note**: All framework-specific paths are aliases to `./handler/http/fetch`. Use the framework-specific path for clarity, or use the generic `fetch` path - both work identically.
...
import { Multipart } from "@visulima/storage/handler/http/hono";
```
