...
You can create draft emails without sending them. This is useful for previewing, saving for later, or testing email content. Draft emails automatically include an `X-Unsent: 1` header to indicate they are drafts.
...
console.log("Draft from:", draft.from);
console.log("Draft to:", draft.to);
console.log("Draft headers:", draft.headers); // Includes X-Unsent: 1

// Send the draft later
const result = await mail.send(draft);
```
...
// Save draft to database, send later, etc.
...
const draft = await mail.draft(message);
// draft.from will be "noreply@example.com"
// draft.headers will include "X-App-Name": "MyApp"
```
