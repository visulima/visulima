...
// Or, different certificates per recipient
const encrypter = createSmimeEncrypter({
    certificates: {
...
- `3des` or `des-ede3-cbc` - Triple DES in CBC mode
...
**S/MIME requires Web Crypto API (Node.js 20+, Bun, Deno) and pkijs/asn1js packages. All runtimes use Node.js APIs via compatibility layers where available. \***S/MIME in Cloudflare Workers: Not supported - requires `node:fs/promises` for file reading. Current implementation expects file paths, not strings. Code would need modification to accept strings instead of file paths.
...
// Store private key in environment variable (must be string, not file path)
const PRIVATE_KEY = env.DKIM_PRIVATE_KEY; // PEM format string
...
const dkimSigner = createDkimSigner({
    domainName: "example.com",
    keySelector: "default",
    privateKey: PRIVATE_KEY, // Must be string, not file:// path
