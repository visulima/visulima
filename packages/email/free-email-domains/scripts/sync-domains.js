...
import FreeEmailSyncManager from "./free-email-sync-manager.js";
...
const main = async () => {
    try {
        ...
        const syncManager = new FreeEmailSyncManager({
            concurrency: 5,
        ...
        // Check for custom exclude list (blacklist.json) file
        const excludePath = join(dirnamePath, "config/blacklist.json");

        // eslint-disable-next-line no-console
        console.log("🚀 Starting domain synchronization...");

        // Run synchronization with optional exclude list
        const result = await syncManager.sync(repositories, excludePath);
        ...
        summaryTable.addRows(
            ...
            ["Provider Dataset Domains", result.stats.providerDomains.toLocaleString()],
            ["Excluded Domains", result.stats.excludeDomains.toLocaleString()],
