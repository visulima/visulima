 ...
 */
// `isErr` is the conventional Result-helper name (parity with unemail / Rust's is_err).
// eslint-disable-next-line unicorn/prevent-abbreviations
export const isErr = <T>(result: Result<T>): boolean => !result.success;
...
export const unwrap = <T>(result: Result<T>): T => {
    ...
    throw new EmailError("result", "Called unwrap() on a failed result", { cause: result.error });
};
