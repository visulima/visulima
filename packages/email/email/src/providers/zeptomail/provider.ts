 ...
 */
const zeptomailProvider: ProviderFactory<ZeptomailConfig> = defineProvider((config: ZeptomailConfig = {} as ZeptomailConfig) => {
    if (!config.token) {
            ...
            try {
                ...
                if (emailOptions.bcc) {
                    if (Array.isArray(emailOptions.bcc)) {
                        payload.bcc = emailOptions.bcc.length === 1 ? (emailOptions.bcc[0] as EmailAddress).email : emailOptions.bcc.map((addr) => addr.email);
                    } else {
