...
- **Hard dependencies (always installed):** `@visulima/disposable-email-domains`, `html-to-text`, `ical-generator`, `lru-cache`, `mime`. Don't move these to peers without a migration plan.
...
- `@visulima/disposable-email-domains` — domain list backing `./validation/disposable-email-domains`.
- Implicit Nx deps (`project.json`): `filesystem/fs`, `filesystem/path`, `email/disposable-email-domains`.
