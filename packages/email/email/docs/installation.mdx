...
The disposable email detection utility is included by default using `@visulima/disposable-email-domains`. The `isDisposableEmail` utility function is available to detect temporary/disposable email addresses. See [Validating Email Addresses](/docs/package/email/getting-started/sending-mail#validating-email-addresses) for usage examples.
