...
const { email: sealed, headers } = await signArc(message, {
...
// `sealed` is the EmailOptions with the three ARC-* headers added; `headers` is the raw header set.
```
...
The signature-base builders `arcMessageSignatureBase(email, options)` and `arcSealBase(aarValue, amsValue, options)` are also exported so an ARC verifier can re-derive the exact bytes that were signed.

> **Note:** like the DKIM signer, the body hash (`bh=`) is computed over `text` + `"\n\n"` + `html` with relaxed canonicalization. For a third-party ARC verifier to accept the seal, the body transmitted on the wire must canonicalize to the same value — apply ARC where the final MIME body is known. Only RSA keys are supported today (Ed25519 is not yet implemented).
