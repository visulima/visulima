...
describe(infobipProvider, () => {
    ...
    describe("branch coverage", () => {
        ...
        it("should send with logger, templateId without variables, and a raw Buffer attachment", async () => {
            expect.assertions(1);

            const makeRequestMock = makeRequest as ReturnType<typeof vi.fn>;

            makeRequestMock.mockResolvedValue({ data: { body: { messages: [{ messageId: "m-1" }] }, statusCode: 200 }, success: true });

            const logger = { debug: vi.fn(), error: vi.fn(), info: vi.fn(), log: vi.fn(), warn: vi.fn() } as unknown as Console;
            const provider = infobipProvider({ apiKey: "test123", logger });
