...
import { generateSelector, labelSection } from "./section-detection";

type SectionRect = { height: number; width: number; x: number; y: number };
...
export type BoundsOverflow = {
    container?: { edges: ("left" | "right" | "top" | "bottom")[]; label: string }; // edges outside nearest container
...
type Candidate = { area: number; bottom: number; label: string; left: number; right: number; selector: string; top: number };

/**
 * Collect visible page elements (2 levels deep from main/body) as spatial candidates.
 */
const collectDOMCandidates = (): Candidate[] => {
    const main = document.querySelector("main") || document.body;
    const results: Candidate[] = [];

    const topLevel = [...main.children] as HTMLElement[];
    ...
    for (const el of roots) {
        ...
        if (Object.hasOwn(el.dataset, "feedbackToolbar"))
            continue;
        ...
        for (const child of [...el.children] as HTMLElement[]) {
            ...
            if (Object.hasOwn(child.dataset, "feedbackToolbar"))
                continue;
 ...
 */
export type ExplicitCandidate = { label: string; rect: SectionRect; selector: string };

const explicitToCandidates = (items: ExplicitCandidate[]): Candidate[] => {
    const { scrollY } = globalThis;

    return items.map(({ label, rect, selector }) => {
 ...
 */
const toViewportEdges = (r: SectionRect) => {
    const { scrollY } = globalThis;
...
export const getSpatialContext = (
    ...
    let containedIn: { _area: number; label: string; selector: string } | null = null;
    ...
    // Skip if the candidate IS the target (rects match closely)
        ...
        if (
            ...
            && (!containedIn || c.area < containedIn._area)
        ) {
            containedIn = { _area: c.area, label: c.label, selector: c.selector };
        }
        ...
        if (hOverlap && c.bottom <= target.top + 5) {
            ...
            if (!above || gap < above._dist) {
                above = { _dist: gap, gap: Math.max(0, gap), label: c.label, selector: c.selector };
            }
        }
        ...
        if (hOverlap && c.top >= target.bottom - 5) {
            ...
            if (!below || gap < below._dist) {
                below = { _dist: gap, gap: Math.max(0, gap), label: c.label, selector: c.selector };
            }
        }
        ...
        if (vOverlap && c.right <= target.left + 5) {
            ...
            if (!left || gap < left._dist) {
                left = { _dist: gap, gap: Math.max(0, gap), label: c.label, selector: c.selector };
            }
        }
        ...
        if (vOverlap && c.left >= target.right - 5) {
            ...
            if (!right || gap < right._dist) {
                right = { _dist: gap, gap: Math.max(0, gap), label: c.label, selector: c.selector };
            }
        }
    }
    ...
    const clean = (n: (NearbyElement & { _dist: number }) | null): NearbyElement | null => {
        ...
        return { gap: n.gap, label: n.label, selector: n.selector };
    };
    ...
    const outOfBounds = detectBoundsOverflow(
        ...
        viewportWidth,
        viewportHeight,
        containedIn ? { _area: containedIn._area, label: containedIn.label, selector: containedIn.selector } : null,
        candidates,
    );
 ...
 */
const detectBoundsOverflow = (
    targetEdges: { bottom: number; left: number; right: number; top: number },
    _targetRect: SectionRect,
    viewportWidth: number,
    viewportHeight: number,
    container: { _area: number; label: string; selector: string } | null,
    ...
    if (targetEdges.left < -2)
        vpOverflow.push("left");

    if (targetEdges.right > viewportWidth + 2)
        vpOverflow.push("right");

    if (targetEdges.top < -2)
        vpOverflow.push("top");

    if (targetEdges.bottom > viewportHeight + 2)
        vpOverflow.push("bottom");
    ...
    if (container) {
        ...
        if (cont) {
            ...
            if (targetEdges.left < cont.left - 2)
                contOverflow.push("left");

            if (targetEdges.right > cont.right + 2)
                contOverflow.push("right");

            if (targetEdges.top < cont.top - 2)
                contOverflow.push("top");

            if (targetEdges.bottom > cont.bottom + 2)
                contOverflow.push("bottom");

            if (contOverflow.length > 0) {
                result.container = { edges: contOverflow, label: container.label };
                hasOverflow = true;
            }
        }
    }
...
const getAlignment = (rect: SectionRect, viewportWidth: number): SpatialContext["alignment"] => {
 ...
 */
const formatAlignment = (alignment: SpatialContext["alignment"]): string => {
    switch (alignment) {
 ...
 */
export const formatSpatialLines = (
    ctx: SpatialContext,
 ...
 */
export const formatPositionSummary = (
    ctx: SpatialContext,
    ...
    if (ctx.above)
        parts.push(`below \`${ctx.above.label}\``);

    if (ctx.below)
        parts.push(`above \`${ctx.below.label}\``);

    if (ctx.left)
        parts.push(`right of \`${ctx.left.label}\``);

    if (ctx.right)
        parts.push(`left of \`${ctx.right.label}\``);

    // Container
    if (ctx.containedIn)
        parts.push(`inside \`${ctx.containedIn.label}\``);
 ...
 */
export const detectGroups = (items: { label: string; rect: SectionRect }[]): LayoutGroup[] => {
    if (items.length < 2)
        return [];
 ...
 */
export const analyzeLayoutPatterns = (
    sections: { currentRect: SectionRect; label: string; originalRect: SectionRect }[],
): string[] => {
    if (sections.length < 2)
        return [];

    const origGroups = detectGroups(sections.map((s) => { return { label: s.label, rect: s.originalRect }; }));
    const currGroups = detectGroups(sections.map((s) => { return { label: s.label, rect: s.currentRect }; }));
    ...
    for (const cg of currGroups) {
        if (cg.labels.every((l) => described.has(l)))
            continue;
 ...
 */
export const getPageLayout = (viewport: { height: number; width: number }): PageLayout => {
    if (typeof document === "undefined")
        return { contentArea: null, viewport };
    ...
    if (roleMain)
        addCandidate(roleMain);

    // Walk 3 levels deep from body to catch framework wrappers
    for (const l1 of [...document.body.children] as HTMLElement[]) {
        addCandidate(l1);

        if (l1.children) {
            for (const l2 of [...l1.children] as HTMLElement[]) {
                addCandidate(l2);

                if (l2.children) {
                    for (const l3 of [...l2.children] as HTMLElement[]) {
                        addCandidate(l3);
                    }
                }
            }
 ...
 */
export const getElementCSSContext = (selector: string): ElementCSSContext | null => {
    if (typeof document === "undefined")
        return null;
 ...
 */
export const formatCSSPosition = (
    rect: SectionRect,
    layout: PageLayout,
): string | null => {
