...
describe("usage `@visulima/dev-toolbar` bin entry", () => {
    it(`should load the MCP server bin and exit cleanly on EOF`, () => {
        expect.assertions(2);

        const result = runBin(join(packageRoot, "bin/mcp.js"), [], packageRoot);
