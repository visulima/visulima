...
Opens a file in the configured editor (uses Vite's `open-in-editor` mechanism, which respects the `LAUNCH_EDITOR` environment variable and falls back to VS Code).
