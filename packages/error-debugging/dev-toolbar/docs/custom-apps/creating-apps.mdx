...
<Callout type="info">
    App components are Preact components rendered inside the Shadow DOM. The JSX pragma `/** @jsxImportSource preact */` is required in every `.tsx` file.
</Callout>
...
const MyApp = ({ helpers }: AppComponentProps): ComponentChildren => {
    ...
    return (
        <div class="p-4 space-y-4">
            <button
                class="px-3 py-1.5 border border-border text-sm cursor-pointer bg-transparent text-foreground"
                onClick={load}
                type="button"
            >
                Load Data
            ...
            {data && (
                <pre class="text-xs font-mono text-foreground/80 bg-foreground/5 p-4 overflow-auto">
                    {data}
                </pre>
            )}
        </div>
...
| `dev-toolbar:more` | Built-in overflow panel |
