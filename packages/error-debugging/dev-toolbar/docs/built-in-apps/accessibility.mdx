...
axe-core is a runtime dependency of `@visulima/dev-toolbar`. It is dynamically imported inside the scan function, so it does **not** add to your bundle size — axe-core is only loaded when the user triggers a scan in development mode.
