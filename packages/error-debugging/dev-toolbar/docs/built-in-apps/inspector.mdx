...
Hold **Cmd** (Mac) or **Ctrl** (Windows/Linux) while hovering to activate pierce mode. This skips through invisible overlays, animation wrappers, and empty container divs to reach the actual content element underneath. Uses a two-pass algorithm: text content first, then smallest visible element.
