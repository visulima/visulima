...
Controls which built-in apps are registered. All optional apps are **disabled by default** — only `viteConfig`, `settings`, and the "more" drawer are shown out of the box. Set an app to `true` to enable it.
...
interface DevToolbarOptions {
     ...
     * Only viteConfig, settings, and the more drawer are shown out of the box.
     */
    apps?: {
        a11y?: boolean;
        assets?: boolean;
