...
```ts title="vite.config.ts"
devToolbar({
    // router.tsx is always part of the client bundle.
    appendTo: /router\.tsx$/,
})
```
...
<Callout type="info">
    Source injection reads the unmodified on-disk file so that both SSR and client builds emit identical
    `data-vdt-source` values.  This prevents React hydration mismatches in SSR frameworks such as TanStack Start.
</Callout>
