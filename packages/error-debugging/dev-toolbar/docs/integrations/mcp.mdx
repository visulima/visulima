...
```
Browser (Dev Toolbar)          MCP Server             AI Agent
─────────────────────          ──────────             ────────
Create annotation    ──save──→ .devtoolbar/    ←──── get_pending_annotations
Capture screenshot   ──save──→ screenshots/    ←──── get_screenshot
                               annotations.json ←──── resolve_annotation
View thread updates  ←─read──  annotations.json ←──── add_thread_message
```
...
Agent: Uses get_pending_annotations → reads context → fixes code → resolve_annotation
```
...
Agent: Uses watch_annotations in a loop → processes each batch → resolves
```
