...
if (globalThis.window !== undefined && globalThis.__VISULIMA_DEVTOOLS__) {
    const exampleApp: DevToolbarApp = {
        ...
        init(canvas) {
            ...
            heading.textContent = "Cloudflare Example";

            const desc = document.createElement("p");

            desc.style.cssText = "margin:0 0 12px;opacity:.7";
            desc.textContent = "Custom app registered in the Cloudflare Workers dev-toolbar demo.";
...
if (globalThis.window !== undefined && globalThis.__DEV_TOOLBAR_HOOK__) {
    ...
    hook.on("devtools:init", () => {
        console.log("[Cloudflare] Dev Toolbar initialized!");
    });

    hook.on("devtools:open", (appId: string) => {
        console.log(`[Cloudflare] App opened: ${appId}`);
    });

    hook.on("devtools:close", () => {
        console.log("[Cloudflare] Dev Toolbar closed");
    });

    hook.addTimelineEvent("custom", {
        data: { message: "App mounted!", platform: "Cloudflare Workers" },
        id: "cloudflare-mount",
        level: "info",
        time: Date.now(),
        title: "Cloudflare App Mounted",
    });
}
