...
// Mock ServerResponse for fetch runtimes.
// This class stubs the full ServerResponse interface so any node-style error
// handler can run unchanged in a fetch context. It is shared by both fetch
// adapters (the HTML path and the negotiated path) so header-introspection
// methods such as `hasHeader`/`removeHeader` behave identically everywhere.
