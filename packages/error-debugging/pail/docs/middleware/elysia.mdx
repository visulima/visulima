...
The Elysia adapter uses `derive` to inject a request-scoped [Wide Event](/docs/package/pail/usage/wide-events) into every handler's context. The event auto-emits on response or error.
