...
The SvelteKit adapter provides `handle` and `handleError` hooks that attach a request-scoped [Wide Event](/docs/package/pail/usage/wide-events) to every request. The event auto-emits when the response is sent or an error occurs.
