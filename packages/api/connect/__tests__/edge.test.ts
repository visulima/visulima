...
describe("edge", () => {
    ...
    it("add() - wraps multiple handlers with zod when a schema precedes the handlers", async () => {
        ...
        const first = vi.fn<(_request: any, _context: any, next: any) => any>((_request: any, _context: any, next: any) => next());
        ...
        const request = { extra: "stripped?", method: "POST", name: "bob", url: "http://localhost/" } as unknown as Request;

        await context.run(request, {});
    ...
    it("add() - rejects with a 422 when the zod schema fails to validate", async () => {
        ...
        const request = { method: "GET", name: 123, url: "http://localhost/" } as unknown as Request;

        await expect(context.run(request, {}), "invalid input rejects").rejects.toMatchObject({ statusCode: 422 });
    ...
    it("add() - keeps two handlers untouched when no schema is provided", async () => {
        ...
        const request = { method: "GET", url: "http://localhost/" } as unknown as Request;
