...
const generateCacheKey = (value: string, options?: CaseOptions & { joiner?: string }): string =>
    ...
    `${value}::${options?.joiner ?? ""}::${options?.locale ?? ""}::${options?.knownAcronyms?.join(",") ?? ""}::${options?.normalize ? "true" : "false"}::${options?.stripAnsi ? "true" : "false"}::${options?.stripEmoji ? "true" : "false"}::${options?.handleAnsi ? "true" : "false"}::${options?.handleEmoji ? "true" : "false"}`;
