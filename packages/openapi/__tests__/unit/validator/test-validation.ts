...
async function testVersion(version) {
    test(`version ${version} works`, async (t) => {
        ...
        assert.equal(version_, version, "petstore spec version matches expected version");
    });
}

test("validator.supportedVersions should be a Set", (t) => {
    assert.equal(Validator.supportedVersions instanceof Set, true, "Validator.supportedVersions is a Set");
});
...
test("empty specification should fail", async (t) => {
    ...
    assert.equal(res.errors, "Cannot find supported swagger/openapi version in specification, version must be a string.", "correct error message");
});
...
test("undefined specification should fail", async (t) => {
    ...
    assert.equal(res.errors, "Cannot find JSON, YAML or filename in data", "correct error message");
});
...
test("yaml specification as string works", async (t) => {
    ...
    assert.equal(valueVersion, "3.0", "yaml validator version matches expected version");
...
test("invalid yaml specification as string gives an error", async (t) => {
  ...
  `;
    ...
    assert.equal(res.errors, "Cannot find JSON, YAML or filename in data", "error message matches expection");
});
...
test("original petstore spec works", async (t) => {
    ...
    const { version } = validator;
    assert.equal(version, "2.0", "original petstore spec version matches expected version");
    const resolvedSpec = validator.resolveRefs();
    assert.equal(resolvedSpec.paths["/pet"].post.parameters[0].schema.required[0], "name", "$refs are correctly resolved");
});
...
test("addSpecRef: no uri and no $id attribute returns an error", (t) => {
    const validator = new Validator();
    assert.rejects(validator.addSpecRef(subSpecYamlFileName), new Error("uri parameter or $id attribute must be a string"), "error message matches expection");
});
...
test("addSpecRef works", async (t) => {
    ...
    assert.equal(validator.specification[inlinedReferences][subSpecUri].components.requestBodies.Pet.required, true);
    assert.equal(validator.specification[inlinedReferences][subSpecUri2].get.summary, "Finds Pets by status");
    const resolvedSpec = validator.resolveRefs();
    assert.equal(resolvedSpec.paths["/pet"].post.requestBody.required, true, "$refs from main spec to sub spec are correctly resolved");
    assert.equal(resolvedSpec.paths["/pet/findByStatus"].get.summary, "Finds Pets by status", "$refs from main spec to sub2 spec are correctly resolved");
});
