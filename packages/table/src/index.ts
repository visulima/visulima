...
/** Describes a cell’s position and span in the table layout. */
export interface LayoutCell extends CellOptions {
    height: number;
    /** True if this cell is a placeholder covering a spanned area */
    isSpanCell?: boolean;
    /** The originating cell that spans multiple rows/columns */
...
/** A helper to track which (column, row) positions are occupied. */
class CellTracker {
    ...
    public findNextFree(columnStart: number, row: number): number {
...
/** Creates placeholder span cells for every covered position except the starting one. */
function createSpanCells(parentCell: LayoutCell): LayoutCell[] {
    ...
    for (let rowIndex = parentCell.y; rowIndex < parentCell.y + parentCell.height; rowIndex++) {
        for (let columnIndex = parentCell.x; columnIndex < parentCell.x + parentCell.width; columnIndex++) {
            if (columnIndex === parentCell.x && rowIndex === parentCell.y) continue;
            spanCells.push({
                content: "",
                height: 1,
                isSpanCell: true,
 ...
 * It is assumed that each row has been “filled” so that its logical width (the sum of colSpans)
 * equals the table’s global column count.
 */
function createTableLayout(rows: CellType[][]): TableLayout {
    ...
    for (const [rowIndex, currentRow] of rows.entries()) {
        // Process each cell in order; the row array is assumed to have a length equal to its logical width.
        for (const [cellIndex, currentCell] of currentRow.entries()) {
            if (currentCell === null) continue; // null cells are assumed covered by a spanning cell
...
/** Computes the real character index in text corresponding to the visible position (ignoring ANSI codes). */
function findRealPosition(text: string, visiblePosition: number): number {
    ...
    while ((matchResult = globalAnsiPattern.exec(text)) !== null) {
        ansiRanges.push({ end: matchResult.index + matchResult[0].length, start: matchResult.index });
    }
    let currentIndex = 0;
    while (currentIndex < text.length) {
        const foundRange = ansiRanges.find((range) => currentIndex >= range.start && currentIndex < range.end);
        if (foundRange) {
            currentIndex = foundRange.end;
            continue;
        }
...
/** Computes the logical width of a row (the sum of its cells’ colSpans, defaulting to 1). */
...
/** Pads a row with null values so that its logical width equals targetWidth. */
...
/**
 * Helper: Returns the underlying “real” cell for grouping purposes.
 * If the cell is a span cell, returns its parent; otherwise returns the cell.
 */
...
/**
 * Helper: Determines if two layout cells are equivalent (i.e. represent the same originating cell).
 */
...
export class Table {
    ...
    // Global logical column count (widest row in terms of colSpans)
    ...
    public toString(): string {
        ...
        // Sort layout cells by row then column.
        this.layout.cells.sort((cellA, cellB) => cellA.y - cellB.y || cellA.x - cellB.x);
            ...
            if (rowIndex < allRows.length - 1) {
                ...
                for (let colIndex = 0; colIndex < this.columnCount; colIndex++) {
                    separatorLine += spanned[colIndex]
                        ...
                        : (borderStyle.joinBody ?? "").repeat(this.columnWidths[colIndex]);
    ...
    private calculateColumnWidths(): number[] {
        ...
        for (const currentRow of allRows) {
            ...
            for (const cell of currentRow) {
                if (currentColumnIndex >= this.columnCount) break;
    ...
    private getIndexOfNearestSpace(text: string, targetIndex: number, searchRight = false): number {
        if (text.charAt(targetIndex) === " ") return targetIndex;
    ...
    private wordWrap(text: string, maxWidth: number): string[] {
        ...
        for (const line of lines) {
            ...
            while ((currentMatch = colorPattern.exec(line)) !== null) {
                formats.push({ index: currentMatch.index, sequence: currentMatch[0] });
            }
            linkPattern.lastIndex = 0;
            while ((currentMatch = linkPattern.exec(line)) !== null) {
                formats.push({
                    index: currentMatch.index,
                    sequence: `\u001B]8;;${currentMatch[1]}\u0007${currentMatch[2]}\u001B\]8;;\u0007`,
                });
            }
            ...
            for (const word of words) {
                ...
                if (lastAnsi) currentLine += lastAnsi;
                const wordStart = plainText.indexOf(word);
                let formattedWord = word;
                for (const format of formats) {
                    if (format.index <= wordStart) {
                        if (format.sequence.startsWith("\u001B[")) lastAnsi = format.sequence;
     ...
     *
     * For each column (0 … columnCount-1), we select the layout cell covering that position.
     * If multiple cells cover the same column, we choose the one that starts on the current row;
     ...
     * For groups where the cell starts on this row, we normalize it (to fix truncate options)
     * and use its content; otherwise, we output blank.
     */
    private renderRow(row: CellType[], columnWidths: number[], rowIndex: number): string[] {
        ...
        for (let columnIndex = 0; columnIndex < this.columnCount; columnIndex++) {
            const coveringCells = this.layout!.cells.filter(
                (layoutCell) =>
            ...
            if (coveringCells.length > 0) {
                ...
                chosenCell =
                    startingCells.length > 0 ? startingCells[0] : coveringCells.reduce((previous, current) => (current.y > previous.y ? current : previous));
            }
        ...
        // Pad each group vertically and horizontally.
        const paddedGroupContents = groupContents.map((lines, groupIndex) => {
            const effectiveWidth = groupWidths[groupIndex];
            const groupCell = groups[groupIndex].cell;
            ...
            for (let index = 0; index < bottomPadding; index++) {
                paddedLines.push(blankLine);
            }
            return paddedLines;
        });
    ...
    private preserveAnsiCodes(text: string, startIndex: number, endIndex: number): string {
        ...
        let currentMatch: RegExpExecArray | null = null;
        globalAnsiPattern.lastIndex = 0;
        while ((currentMatch = globalAnsiPattern.exec(text)) !== null) {
            if (currentMatch.index > endIndex) break;
            const code = currentMatch[0];
            if (code === "\u001B[0m") {
                if (currentMatch.index < endIndex) {
                    openCodes.length = 0;
                }
            } else if (code.startsWith("\u001B[") && currentMatch.index < endIndex) {
                openCodes.push(code);
            }
        }
    ...
    private truncate(text: string, maxWidth: number, options: Required<TruncateOptions>): string {
        ...
        if (maxWidth === 1) return options.truncationCharacter;
        ...
        const truncatedLines = lines.map((line) => {
            ...
            if (lineLength <= maxWidth) return line;
            if (options.position === "start") {
                ...
                if (options.space) truncationCharacter += " ";
            ...
            if (options.position === "middle") {
                if (options.space) truncationCharacter = ` ${truncationCharacter} `;
                ...
                const firstHalfEnd = findRealPosition(line, halfWidth);
                ...
                return this.preserveAnsiCodes(line, 0, firstHalfEnd) + truncationCharacter + this.preserveAnsiCodes(line, secondHalf, line.length);
            }
            if (options.position === "end") {
                ...
                if (options.space) truncationCharacter = ` ${truncationCharacter}`;
                const endIndex = findRealPosition(line, maxWidth - stringWidth(truncationCharacter));
                return this.preserveAnsiCodes(line, 0, endIndex) + truncationCharacter;
            }
