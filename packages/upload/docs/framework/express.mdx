...
Use Visulima upload with Express.js for file uploads and on-demand image transformations. For modern frameworks like Hono or Cloudflare Workers, see the [fetch method documentation](#fetch-method).
...
app.use("/files", multipart.upload, (req, res) => {
  if (req.file) {
    res.json({
      id: req.file.id,
      filename: req.file.filename,
      size: req.file.size,
      url: `/files/${req.file.id}`
    });
  } else {
...
**Note**: For Express applications, the traditional `upload()` middleware is recommended for simplicity. The `fetch` method is primarily designed for Web API frameworks like Hono, Cloudflare Workers, and Deno.
...
```ts
import express from "express";
import { DiskStorage, ImageTransformer, Multipart } from "@visulima/upload";
