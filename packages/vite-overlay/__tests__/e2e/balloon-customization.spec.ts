...
test.describe("Balloon Customization", () => {
    ...
    test("should expose global overlay API", async ({ page }) => {
        ...
        const apiExists = await page.evaluate(() => {
            return typeof window.__visulima_overlay__ !== "undefined";
        });

        expect(apiExists).toBe(true);

        // Check if sendError method exists
        const hasSendError = await page.evaluate(() => {
            return typeof window.__visulima_overlay__?.sendError === "function";
        });
    ...
    test("should have open and close methods in API", async ({ page }) => {
        ...
        // Use API to close
        await page.evaluate(() => {
            window.__visulima_overlay__?.close();
        });
    ...
    test("should have getInstance method in API", async ({ page }) => {
        ...
        const instance = await page.evaluate(() => {
            return window.__visulima_overlay__?.getInstance();
        });
    ...
    test("should apply custom balloon styles when configured", async ({ page }) => {
        ...
        const balloon = page.locator("#__v_o__balloon");
        const styles = await balloon.evaluate((el) => {
