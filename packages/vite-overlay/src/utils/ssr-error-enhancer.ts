...
export const enhanceViteSsrError = async (rawError: unknown, server: ViteDevServer): Promise<Error> => {
    ...
    if (importName) {
        ...
        if (fileContents && topFile) {
            ...
            const lineIdx = fileLines.findIndex((ln) => ln.includes(importName || ""));
            if (lineIdx !== -1) {
                const lineText = fileLines[lineIdx] ?? "";
                const col = Math.max(0, lineText.indexOf(importName || ""));
                (error as any).loc = { file: topFile, line: lineIdx + 1, column: col + 1 };
            }
        }
    }
    ...
    if (fileId && /\.mdx$/i.test(String(fileId)) && /syntax error/i.test(error.message)) {
        (error as any).hint = (error as any).hint || "MDX detected without an appropriate integration. Install and configure the MDX plugin for Vite/your framework.";
    }
    ...
    if (globPattern) {
        ...
        (error as any).hint = (error as any).hint || "Ensure your glob follows the expected syntax and matches existing files. Avoid unintended special characters.";

        if (fileContents && topFile) {
            const fileLines = fileContents.split("\n");
            const lineIdx = fileLines.findIndex((ln) => ln.includes(globPattern || ""));
            if (lineIdx !== -1) {
                const lineText = fileLines[lineIdx] ?? "";
