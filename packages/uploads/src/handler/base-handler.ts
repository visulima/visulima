...
import type {
    AsyncHandler, Handlers, MethodHandler, ResponseFile, UploadOptions,
} from "./types";
...
abstract class BaseHandler<TFile extends UploadFile, Request extends IncomingMessage = IncomingMessage, Response extends ServerResponse = ServerResponse>
    ...
    public upload = (request: Request, response: Response, next?: () => void): void => {
        ...
        handler
            ...
            .then(async (file: ResponseFile<TFile> | TFile[] | PaginationResult<TFile>): Promise<void> => {
                ...
                if (["HEAD", "OPTIONS"].includes(request.method as string)) {
                ...
                } else if (request.method === "GET" && (Array.isArray(file) || (file as PaginationResult<TFile>).data !== undefined)) {
                    (request as IncomingMessageWithBody).body = file;
                    ...
                    if (typeof next === "function") {
                        ...
                        this.send(response, { statusCode, headers, body: basicFile });
                    }
                } else {
                    const { statusCode, headers, ...basicFile } = file as ResponseFile<TFile>;
     ...
     */
    public async get(request: Request, response: Response): Promise<ResponseFile<TFile> | PaginationResult<TFile> | TFile[]>;

    // eslint-disable-next-line @typescript-eslint/no-unused-vars,consistent-return
    public async get(request: Request & { originalUrl?: string }, _response: Response): Promise<ResponseFile<TFile> | PaginationResult<TFile> | TFile[]> {
     ...
     */
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    public async list(request: Request, _response: Response): Promise<PaginationResult<TFile> | TFile[]> {
        ...
        if (page !== undefined && limit !== undefined) {
            return paginate(Number(page), Number(limit), list.length, list);
        }
