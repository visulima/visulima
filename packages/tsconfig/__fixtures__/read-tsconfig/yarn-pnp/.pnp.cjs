...
function $$SETUP_STATE(hydrateRuntimeState, basePath) {
    ...
    "packageRegistryData": [\
      ...
      ["tsconfig-package", [\
        ["file:../tsconfig-package#../tsconfig-package::hash=abb316&locator=root-workspace-0b6124%40workspace%3A.", {\
          "packageLocation": "./.yarn/cache/tsconfig-package-file-23ef5ea9d0-1dd2cff682.zip/node_modules/tsconfig-package/",\
          "packageDependencies": [\
            ["tsconfig-package", "file:../tsconfig-package#../tsconfig-package::hash=abb316&locator=root-workspace-0b6124%40workspace%3A."]\
          ],\
          "linkType": "HARD"\
...
function fromPortablePath(p) {
    ...
    else if ((uncPortablePathMatch = p.match(UNC_PORTABLE_PATH_REGEXP))) p = `\\\\${uncPortablePathMatch[1] ? `.\\` : ``}${uncPortablePathMatch[2]}`;
    else return p;
    return p.replace(/\//g, `\\`);
}
function toPortablePath(p) {
    ...
    else if ((uncWindowsPathMatch = p.match(UNC_WINDOWS_PATH_REGEXP))) p = `/unc/${uncWindowsPathMatch[1] ? `.dot/` : ``}${uncWindowsPathMatch[2]}`;
    return p;
}
...
var __defNormalProp$4 = (obj, key, value) =>
    key in obj ? __defProp$4(obj, key, { enumerable: true, configurable: true, writable: true, value }) : (obj[key] = value);
...
async function copyPromise(destinationFs, destination, sourceFs, source, opts) {
    ...
    const updateTime =
        typeof destinationFs.lutimesPromise === `function` ? destinationFs.lutimesPromise.bind(destinationFs) : destinationFs.utimesPromise.bind(destinationFs);
    await copyImpl(
        prelayout,
        postlayout,
        updateTime,
...
async function copyImpl(prelayout, postlayout, updateTime, destinationFs, destination, sourceFs, source, opts) {
    ...
    if (
        updated ||
        ((_a = destinationStat == null ? void 0 : destinationStat.mtime) == null ? void 0 : _a.getTime()) !== mtime.getTime() ||
        ((_b = destinationStat == null ? void 0 : destinationStat.atime) == null ? void 0 : _b.getTime()) !== atime.getTime()
    ) {
...
var __defNormalProp$3 = (obj, key, value) =>
    key in obj ? __defProp$3(obj, key, { enumerable: true, configurable: true, writable: true, value }) : (obj[key] = value);
...
class ZipFS extends BasePortableFakeFS {
    ...
    getBufferAndClose() {
        ...
        try {
            ...
            if (this.libzip.source.open(this.lzSource) === -1) throw this.makeLibzipError(this.libzip.source.error(this.lzSource));
            if (this.libzip.source.seek(this.lzSource, 0, 0, this.libzip.SEEK_END) === -1) throw this.makeLibzipError(this.libzip.source.error(this.lzSource));
    ...
    prepareCopyFile(sourceP, destP, flags = 0) {
        ...
        if ((flags & fs.constants.COPYFILE_FICLONE_FORCE) !== 0) throw ENOSYS(`unsupported clone operation`, `copyfile '${sourceP}' -> ${destP}'`);
    ...
    async appendFilePromise(p, content, opts) {
        ...
        else if (typeof opts.flag === `undefined`) opts = __spreadValues$3({ flag: `a` }, opts);
...
class VirtualFS extends ProxiedFS {
    static makeVirtualPath(base, component, to) {
        ...
        if (!ppath.basename(component).match(VALID_COMPONENT)) throw new Error(`Assertion failed: Virtual components must be ended by an hexadecimal hash`);
...
function applyPatch(pnpapi, opts) {
    ...
    require$$0.Module._resolveFilename = function (request, parent, isMain, options) {
        ...
        if (!enableNativeHooks) return originalModuleResolveFilename.call(require$$0.Module, request, parent, isMain, options);
...
- `)}`;
    ...
    process.emit = function (name, data, ...args) {
        if (
            ...
            (data.message.includes(`--experimental-loader`) || data.message.includes(`Custom ESM Loaders is an experimental feature`))
        )
            return false;
...
var __defNormalProp$1 = (obj, key, value) =>
    key in obj ? __defProp$1(obj, key, { enumerable: true, configurable: true, writable: true, value }) : (obj[key] = value);
...
function makeApi(runtimeState, opts) {
    ...
    function trace(entry) {
        ...
        if (error) console.error(c(`31;1`, `\u2716 ${(_d = entry.error) == null ? void 0 : _d.message.replace(/\n.*/s, ``)}`));
        else console.error(c(`33;1`, `\u203C Resolution`));
        if (entry.args.length > 0) console.error();
        for (const arg of entry.args) console.error(`  ${c(`37;1`, `In \u2190`)} ${nodeUtils.inspect(arg, { colors, compact: true })}`);
        if (entry.result) {
            console.error();
    ...
    function isDependencyTreeRoot(packageLocator) {
        ...
        for (const dependencyTreeRoot of runtimeState.dependencyTreeRoots)
            if (dependencyTreeRoot.name === packageLocator.name && dependencyTreeRoot.reference === packageLocator.reference) return true;
        return false;
    }
            ...
            if (containingPackage) {
                ...
                try {
                    ...
                    if ((err == null ? void 0 : err.code) === `ENOENT`) {
                        ...
                        const readableError = (
                            (_b = (_a = err == null ? void 0 : err.message) != null ? _a : err) != null ? _b : `empty exception thrown`
    ...
    return {
        ...
        getAllLocators() {
            ...
            for (const [name, entry] of packageRegistry)
                for (const reference of entry.keys()) if (name !== null && reference !== null) locators.push({ name, reference });
            return locators;
        },
...
`);
        ...
        do {
            ...
            if (opts.fakeFs.existsSync(cjsCandidate) && opts.fakeFs.statSync(cjsCandidate).isFile()) return addToCacheAndReturn(start, curr, cjsCandidate);
...
const defaultApi = Object.assign(
    ...
    }),
    {
        makeApi: (_a) => {
            var _b = _a,
                { basePath = void 0, fakeFs = dynamicFsLayer, pnpapiResolution = defaultPnpapiResolution } = _b,
