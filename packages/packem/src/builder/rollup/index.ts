...
export async function rollupBuild(ctx: BuildContext) {
    ...
    if (ctx.options.declaration) {
        rollupOptions.plugins = [rollupOptions.plugins, dts(ctx.options.rollup.dts), removeShebangPlugin()];
...
const getChunkFilename = (ctx: BuildContext, chunk: PreRenderedChunk, ext: string) => {
