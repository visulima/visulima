...
export const swcTarget = (): Plugin => {
    ...
    return {
        ...
        esbuildOptions(options) {
            if (typeof options.target === "string" && TARGETS.includes(options.target as any)) {
        ...
        async renderChunk(code, info) {
            ...
            if (!swc) {
                throw new PrettyError(`@swc/core is required for ${target} target. Please install it with \`npm install @swc/core -D\``);
            }
