...
describe.runIf(globalThis.window === undefined)("inspect with colors option", () => {
    it("should correctly colorize a Date object when 'colors' is true", () => {
        expect.assertions(1);

        expect(inspect(new Date(1_475_318_637_123), { colors: true, stylize })).toBe("\u001B[35m2016-10-01T10:43:57.123Z\u001B[39m");
    });

    it("should correctly colorize a null value when 'colors' is true", () => {
        expect.assertions(1);

        expect(inspect(null, { colors: true, stylize })).toBe("\u001B[1mnull\u001B[22m");
    });

    it("should correctly colorize a RegExp object when 'colors' is true", () => {
        expect.assertions(1);

        expect(inspect(/abc/, { colors: true, stylize })).toBe("\u001B[31m/abc/\u001B[39m");
    });

    it("should correctly colorize an undefined value when 'colors' is true", () => {
        expect.assertions(1);

        expect(inspect(undefined, { colors: true, stylize })).toBe("\u001B[90mundefined\u001B[39m");
    });

    it("should correctly colorize a BigInt value when 'colors' is true", () => {
        expect.assertions(1);

        expect(inspect(1n, { colors: true, stylize })).toBe("\u001B[33m1n\u001B[39m");
    });

    it("should correctly colorize a string value when 'colors' is true", () => {
        expect.assertions(1);

        expect(inspect("abc", { colors: true, stylize })).toBe("\u001B[32m'abc'\u001B[39m");
    });

    it("should correctly colorize a number value when 'colors' is true", () => {
        expect.assertions(1);

        expect(inspect(3.141, { colors: true, stylize })).toBe("\u001B[33m3.141\u001B[39m");
    });

    it("should correctly colorize boolean values when 'colors' is true", () => {
        expect.assertions(2);
    ...
    it("should correctly colorize NaN when 'colors' is true", () => {
        expect.assertions(1);

        expect(inspect(Number.NaN, { colors: true, stylize })).toBe("\u001B[33mNaN\u001B[39m");
    });

    it("should correctly colorize a function when 'colors' is true", () => {
        expect.assertions(1);
    ...
    it("should correctly colorize positive infinity when 'colors' is true", () => {
        expect.assertions(1);

        expect(inspect(Number.POSITIVE_INFINITY, { colors: true, stylize })).toBe("\u001B[33mInfinity\u001B[39m");
    });

    it("should correctly colorize negative infinity when 'colors' is true", () => {
        expect.assertions(1);
    ...
    it.skip("should colorize an HTML element with cyan, attribute names in yellow, and attribute values in green", () => {
        expect.assertions(1);
