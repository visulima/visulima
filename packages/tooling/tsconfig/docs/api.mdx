...
```typescript
function readTsConfig(path: string, options?: { tscCompatible?: boolean }): Promise<TsConfig>;
```
...
```typescript
function writeTsConfig(config: TsConfig, options?: { cwd?: string }): Promise<void>;
```
