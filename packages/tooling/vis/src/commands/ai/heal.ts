...
const renderProposalDiff = (proposal: FixProposal, workspaceRoot: string, cwd: string | undefined): string => {
    ...
    for (const [index, patch] of proposal.patches.entries()) {
        ...
        const lines: string[] = [`**[${String(index + 1)}] \`${displayPath}\`**`];
...
const renderCommentBody = (proposal: FixProposal, failureContext: FailureContext, workspaceRoot: string, sha: string | undefined): string => {
    const header: string[] = [
        "## vis ai heal — proposed fix",
        "",
        `Failing task: \`${failureContext.taskId}\` (provider: \`${proposal.provider}\`, confidence: \`${proposal.confidence}\`)`,
    ];
    ...
    const footer: string[] = [
        "### Validation",
        `Re-ran \`${failureContext.taskId}\` after applying the patch on the CI runner — task **passed**.`,
        "",
        "### Apply locally",
        "```sh",
        `vis ai fix ${failureContext.taskId} --apply`,
        "```",
        "",
        "---",
        "",
        "_Auto-generated by `vis ai heal`. Auto-commit is on the roadmap; for now the patch lives in this comment and is yours to accept or reject._",
    ];
