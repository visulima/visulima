...
import type { IgnoreDecision } from "../ci-ignore-helpers";
import {
...
const execute = async ({ argument, logger, options, visConfig, workspaceRoot }: Toolbox<Console, IgnoreOptions>): Promise<void> => {
    ...
    if (!project) {
        return emit(decideBuild("", "missing-project-argument", "Missing project argument. Usage: vis ci ignore <project>"));
    }
