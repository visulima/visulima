        ...
        return {
            targets: {
                build: {
                    ...
                    // `matchedConfigs[0]` is always defined here: vite has no
                    // `fallbackDependency`, so the registry skips this detector
                    // unless at least one config file matched.
                    inputs: ["{projectRoot}/src/**/*", `{projectRoot}/${matchedConfigs[0]!}`, "{projectRoot}/package.json"],
