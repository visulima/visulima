...
import {
    ...
    type AdvisoryDbStatus,
    type AdvisoryQueryResult,
    type NativeVulnerabilityJs,
} from "#native";
...
const BUILTIN_ALLOWED_HOSTS = new Set<string>([
...
export class AdvisorySourceNotAllowedError extends Error {
    ...
    constructor(host: string) {
        super(
            `Advisory source host '${host}' is not in the built-in allowlist. `
            + "Add it to `security.audit.advisories.allowedHosts` if intentional.",
        );
 ...
 */
export function resolveAdvisoryDbPath(workspaceRoot: string): string {
 ...
 */
export function validateAdvisorySource(source: string, allowedHosts?: string[]): URL {
    let url: URL;
    try {
        url = new URL(source);
    } catch {
 ...
 */
export async function syncAdvisories(options: SyncOptions): Promise<SyncResult> {
    ...
    const zipPath = `${dbPath}.${options.ecosystem}.${process.pid}.${Math.random().toString(36).slice(2)}.zip.tmp`;
...
export function queryAdvisories(
...
): Map<string, SecurityVulnerability[]> {
    ...
    for (const [index, packageInfo] of packages.entries()) {
        ...
        out.set(
            packageInfo.name,
            hit.vulnerabilities.map(toSecurityVulnerability),
        );
    }
    return out;
}

export async function getAdvisoryStatus(workspaceRoot: string, dbPath?: string): Promise<AdvisoryDbStatus> {
    const path = dbPath ?? resolveAdvisoryDbPath(workspaceRoot);
    return nativeStatus(path);
}

async function readStoredEtag(dbPath: string, ecosystem: string): Promise<string | null> {
    try {
        await stat(dbPath);
    } catch {
        return null;
...
function toSecurityVulnerability(v: NativeVulnerabilityJs): SecurityVulnerability {
    return {
...
function normalizeSeverity(s: string): SecurityVulnerability["severity"] {
...
async function safeFetch(url: URL, init: RequestInit): Promise<Response> {
    try {
        ...
        throw new AdvisorySyncNetworkError(
            url.toString(),
