...
const checkPmNativeConfigDrift = (config: VisConfig, pm: PackageManagerName, workspaceRoot: string): DriftReport => {
    ...
    if (security.allowBuilds && (pm === "pnpm" || pm === "bun")) {
        const visApproved = new Set(
            Object.entries(security.allowBuilds)
                .filter(([, v]) => v)
                .map(([k]) => k),
        );
    ...
    if (security.minimumReleaseAge !== undefined) {
        ...
        if (!yarnClassic && snapshot.minReleaseAgeMinutes !== security.minimumReleaseAge) {
            report.minReleaseAge = { pm: snapshot.minReleaseAgeMinutes, vis: security.minimumReleaseAge };
            report.hasDrift = true;
        }
    }

    if (security.minimumReleaseAgeExclude && (pm === "pnpm" || pm === "bun")) {
        const visSet = new Set(security.minimumReleaseAgeExclude);
