...
Cache task results between CI runs. The path is the task cache directory —
`node_modules/.cache/vis` by default, or whatever
`taskRunner.cacheDirectory` is set to. It is **not** `.vis/`, which holds
run state and tracked files; see [Caching](/guides/caching) for the
distinction.
...
Keying by job keeps parallel jobs from overwriting each other's archive;
the `restore-keys` prefixes let a cold job start from the most recent
compatible archive instead of from nothing.
...
concurrent jobs overwrite each other and every job starts cold.
...
This assumes the cache has been pointed somewhere the job can write:
...
That matters when `node_modules` is restored from a `policy: pull` cache —
the default cache directory lives inside it, so everything vis writes there
is discarded at job end and the cache never hits. See
...
- **GitLab CI** — `image:`, `services:`, and `include: { project, ref }` blocks in `.gitlab-ci.yml` plus anything under `.gitlab/ci/`.
