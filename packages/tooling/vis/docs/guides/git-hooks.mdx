...
These flags map to env vars (`VIS_HOOK_ALL_FILES`, `VIS_HOOK_FROM_REF`, `VIS_HOOK_TO_REF`) that the bundled runner picks up to swap `git diff --cached` for `git ls-files` or `git diff <from> <to>`.
