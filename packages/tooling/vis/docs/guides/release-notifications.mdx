...
const notifiable = result.published.filter(
    (p) => !alreadyNotified.has(`${p.name}@${p.version}`),
);
...
- **You want to gate the release on chat approval.** Notifications fire *after* publish. They cannot block. For human-in-the-loop approval, see [staged publishing](./release-staged-publishing.mdx).
