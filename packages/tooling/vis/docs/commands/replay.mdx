...
Each `vis run --summarize` writes a JSON summary into `.task-runner/runs/<id>.json` and overwrites `.task-runner/last-summary.json`. `vis replay` reads from those files only — it never re-executes tasks. To make a run replayable later, pass `--summarize` (or set it as a default in CI). The most recent run is always available without `--summarize` because `last-summary.json` is written for every run.
...
```json
{
  ...
  "tasks": [
    { "taskId": "@app/api:test", "cacheStatus": "MISS", "duration": 234, "exitCode": 1, "hash": "abc…", "dependencies": ["@app/api:db"], "startTime": "…", "endTime": "…" }
  ]
}
```
