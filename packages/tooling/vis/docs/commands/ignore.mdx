...
```bash
vis ignore <project> [options]
```

## Exit codes

`vis ignore` uses **inverted** exit codes so it matches the contract Vercel and Netlify expect from an ignore hook:
...
# Explicit base ref
vis ignore my-app --base "$VERCEL_GIT_PREVIOUS_SHA"

# Emit the decision as JSON (pipe into jq in GitHub Actions)
vis ignore my-app --json

# Normal exit semantics for local testing
vis ignore my-app --exit-zero-on-build

# Debug the decision path
vis ignore my-app --verbose
```
...
`vis ignore` probes these env vars in order. The first non-empty match wins, unless `--base` is passed explicitly.
...
- [`vis affected`](./affected) — run a target only on affected projects (what `vis ignore` is built on)
