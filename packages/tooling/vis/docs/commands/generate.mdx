...
- `{{ var }}`, `{{ obj.field }}`, `{{ var | filter | filter(arg) }}`
- `{% if expr %}` / `{% else %}` / `{% endif %}` — `expr` may be a variable, `not var`, or `lhs == "rhs"` / `!=`.
- `{% for x in collection %}` / `{% endfor %}` — `collection` must be an array.
- `{% include "name" %}` — pulls in a partial (any file whose path contains `partial`).
