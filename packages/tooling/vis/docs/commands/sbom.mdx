...
Generates a CycloneDX 1.6 Software Bill of Materials (SBOM) for the current workspace, capturing every workspace project, every reachable registry dependency, per-version licenses, and the dependency graph.
...
| `--format`       | `json`             | Output format: `json` (CycloneDX 1.6 JSON) or `xml`.                                       |
| `--output`       | `./sbom.cdx.json`  | Output file path. Use `-` to write to stdout.                                              |
| `--include-dev`  | `false`            | Include `devDependencies` in the closure walk. Default is production-only, per industry practice. |

## What's in the BOM

Every emitted BOM passes the vendored CycloneDX 1.6 JSON Schema (`bom-1.6.schema.json`). Each `components[]` entry carries:
...
| Yarn Berry      | `yarn.lock` (v2+)  | ✅ Full except integrity hashes — Berry uses XXH64 (`checksum:`), which is not a cryptographic hash and is outside CycloneDX 1.6's `HashAlgorithm` enum. Berry components emit without a `hashes` entry; all other fields are populated normally. |
