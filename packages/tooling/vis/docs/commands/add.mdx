...
Add packages using the detected package manager. Before installing, vis runs a **typosquat check** against a curated blocklist of known malicious package names, an offline **marshall pipeline** (author, provenance, metadata, downloads, expired-domains, new-bin, archived-repo), and an optional **Socket.dev security scan**.
