...
> **Not the same as `vis lint --custom-types`.** `vis doctor` verifies the *installed* runtime matches `engines.node` on this machine. `vis lint --custom-types` verifies that every package.json in the workspace *declares* the same `engines.node`. Run doctor locally; run lint in CI. See [`vis lint`](./lint.mdx#overlap-with-vis-doctor) for the workspace-side check.
