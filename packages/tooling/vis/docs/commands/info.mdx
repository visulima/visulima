...
Show registry metadata for any published package — versions, dependencies, maintainers, dist-tags, license, repository, and more. Wraps the detected package manager's native lookup (`npm view`, `pnpm view`, `yarn info`, `yarn npm info`, `bun pm view`) so the right spelling is used without having to remember each tool's quirks.
...
`vis info` detects the active package manager (pnpm, npm, yarn v1, yarn berry, or bun) and forwards the query using the correct command shape for that tool:
