...
Runs `deps` + `lint-staged` (does **not** auto-run turborepo/nx/moon — those are opt-in). Omitting the `<type>` argument defaults to `all`:

```bash
vis migrate        # equivalent to vis migrate all
vis migrate all
```
