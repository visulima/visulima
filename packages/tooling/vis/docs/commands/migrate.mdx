...
Strips the `sherif` dep / scripts / `package.json#sherif` block and surfaces a positive `vis lint --<rule>` command suggestion (vis has no global rule-disable, so the migration translates sherif's `ignore-rules` into the explicit "rules to keep enabled" form):
...
**Sherif → vis lint mapping:**

| Sherif rule (in `ignore-rules`) | vis lint flag                                        |
| ------------------------------- | ---------------------------------------------------- |
...
- `ignore-rules` (when set) → suggested replacement script: `vis lint --<rule> --<rule> ...` listing every covered rule _except_ the disabled ones.
...
- Pre-commit hooks and CI workflows referencing `sherif` are flagged but not auto-rewritten — the reviewer chooses the right `vis lint` flag set.
