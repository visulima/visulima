...
Runs every detected formatter against the workspace. Defaults to write mode (`--fix`); pass `--check` for a dry-run that lists files that would change without modifying them. Designed to add more formatters (dprint, oxfmt, biome, …) over time without changing the command surface.

Phase 1 ships with the **Prettier** adapter. More follow in subsequent phases.
...
When multiple fmt adapters are detected, files are routed by extension via the registry's `routeFilesByExtension` helper. Override per-extension routing through `fmt.extensionOverrides` in `vis.config.ts` once additional adapters land (e.g. send `.md` to `dprint` even when `prettier` also handles it).
