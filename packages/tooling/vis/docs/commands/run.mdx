...
| `--flaky`           |       | `false` | Show flaky task report after execution               |
...
| `osType`              | `string \| string[]`              |                | Restrict to `linux`, `macos`, `windows` |
...
Pass `--flaky` to see which tasks have historically flaky outcomes:

```bash
vis run :test --summarize --flaky
```

The report reads `.task-runner/runs/*.json` summaries (created by `--summarize`) and shows tasks that have both successes and failures across runs:
