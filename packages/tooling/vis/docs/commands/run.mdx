...
| `--summarize`        |       | `false`        | Generate a JSON run summary at `.task-runner/runs/<id>.json` (also enables historical diffs for `vis cache why` / `vis replay`)                                                                          |
...
| `--last-details`     |       | `false`        | Render the most-recent run's saved summary (`.task-runner/last-summary.json`) and exit without executing any tasks. Use for "what happened in the last build?" without re-running.                       |
