...
```dockerfile
# Linux / macOS images
RUN curl -fsSL https://visulima.dev/install.sh \
    | bash -s -- --yes --manager=proto --no-toolchain-install
```

```dockerfile
# Windows images (PowerShell)
RUN powershell -Command "& ([scriptblock]::Create((irm 'https://visulima.dev/install.ps1'))) -Yes -Manager proto -NoToolchainInstall"
```
