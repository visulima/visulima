...
> interactive picker is skipped — the script installs **the latest Node LTS directly**.
> Pass `--manager=proto|fnm|mise|volta` (or set `VIS_MANAGER`) to take the
> version-manager path instead, or `VIS_NODE_MAJOR` to install a different
> Node major.
