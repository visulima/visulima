...
export default defineConfig({
    toolchain: {
        ...
        // true, `vis run` / `vis ci` call the right manager on engines.node
        // mismatch instead of bailing.
        autoInstall: true,
...
`vis` is a Node CLI, so a truly empty machine can't run it until something installs Node. The supported bootstrap is the published install script:
...
CI agents / Docker layers should pass `--yes` and `--manager=<name>`:
...
If Node is on PATH the script skips the manager install step and goes straight to `npm install -g @visulima/vis`. In that case you can also just run that command directly — `install.sh` is a convenience, not a requirement.

### Windows

Run `npm install -g @visulima/vis` from a shell where Node is already available (for example after installing [volta](https://volta.sh/) or [fnm](https://github.com/Schniz/fnm)). The install script assumes POSIX paths and is not supported on native Windows cmd/PowerShell.
