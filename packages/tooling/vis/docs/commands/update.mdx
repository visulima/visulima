...
| `--no-marshall-check`          |       | `false`  | Skip the offline marshall pipeline when explicit package arguments are supplied (author, provenance, metadata, downloads, expired-domains, new-bin, archived-repo). Blanket `vis update` never runs the pipeline. |
