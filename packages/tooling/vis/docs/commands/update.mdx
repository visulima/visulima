...
This is separate from `security.policies.first_seen.minutes` (which applies at install time). The update setting is **not enabled by default** — all published versions are eligible for updates unless you opt in.
