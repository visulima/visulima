...
- **`vis doctor`** verifies the _installed_ runtime matches `engines.node` (machine-side: "is my Node binary the right version?").
- **`vis lint --custom-types`** verifies all packages _declare_ the same `engines.node` (workspace-side: "do all my package.json files agree?").
