...
Ships with adapters for **oxlint**, **biome**, and **ESLint**. The orchestrator runs each one that's present in the workspace.
...
When multiple lint adapters are detected, they run in this default precedence: `oxlint → biome → eslint`. The Rust-native oxlint pass runs first as a fast pre-filter, biome handles its bespoke rule set, and ESLint catches what the others don't. Override with `lint.order` in `vis.config.ts`.
