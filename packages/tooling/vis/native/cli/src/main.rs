...
fn main() {
    ...
    match first {
        ...
        "x" => x::run(&args),
        // pm translate-and-exec family: detect PM, map flags, exec. Pass the FULL
        // argv so the config-delegation path can replay it.
        "remove" | "dedupe" | "link" | "unlink" | "outdated" | "why" => pm_family::run(first, &args),
