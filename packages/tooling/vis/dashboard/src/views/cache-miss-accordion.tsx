...
const changeSymbol = (change: HashDiffEntry["change"]) => {
    switch (change) {
        ...
        case "removed": {
            return { symbol: "-", color: "text-accent" };
        }
        default: {
            return { symbol: "~", color: "text-warning" };
        }
    }
};
...
export const CacheMissAccordion = ({ runId, taskId }: CacheMissAccordionProps) => {
    ...
    const { reason, previousHash, previousRunStartTime, currentHash, entries } = diffQuery.data;
            ...
            {entries.length === 0 ? (
                <div className="border border-dashed border-border2 px-4 py-3">
                    ...
                    <p className="text-[12px] text-faint">
                        ...
                        <code className="nd-mono px-1 text-fg">vis cache prune</code> or the size limit.
                    </p>
                </div>
            ) : (
                <div className="border border-border">
                    {entries.map((entry, index) => {
                        const { symbol, color } = changeSymbol(entry.change);
                        return (
                            <div
