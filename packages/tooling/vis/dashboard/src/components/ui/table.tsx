...
const TableRow = ({ className, ...props }: React.HTMLAttributes<HTMLTableRowElement>) => (
    <tr className={cn("transition-colors hover:bg-row-hover data-[state=selected]:bg-row-hover [&>td]:border-b [&>td]:border-border", className)} {...props} />
);

const TableHead = ({ className, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) => (
    <th
        className={cn("nd-mono h-10 bg-bg px-4 text-left align-middle text-[11px] uppercase tracking-[0.11em] text-faint border-b border-border2", className)}
        {...props}
    />
);
