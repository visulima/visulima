...
describe(extractLicenseChoice, () => {
    it("should extract a string-form licence as an SPDX id", () => {
        ...
        expect(extractLicenseChoice({ license: "MIT" })).toEqual([{ license: { id: "MIT" } }]);
    });

    it("should treat SPDX expressions as expressions", () => {
        expect.assertions(1);

        expect(extractLicenseChoice({ license: "(MIT OR Apache-2.0)" })).toEqual([{ expression: "(MIT OR Apache-2.0)" }]);
    });

    it("should handle the legacy `license: { type }` object form", () => {
        expect.assertions(1);

        expect(extractLicenseChoice({ license: { type: "Apache-2.0" } })).toEqual([{ license: { id: "Apache-2.0" } }]);
    });

    it("should handle the legacy `licenses: []` array form", () => {
        expect.assertions(1);

        expect(extractLicenseChoice({ licenses: [{ type: "ISC" }] })).toEqual([{ license: { id: "ISC" } }]);
    });

    it("should fall back to a named licence when the SPDX id is unknown", () => {
        expect.assertions(1);

        expect(extractLicenseChoice({ license: "Proprietary" })).toEqual([{ license: { name: "Proprietary" } }]);
    });
