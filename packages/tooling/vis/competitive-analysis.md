...
Analysis date: 2026-04-28 (Vite+ §1.4 refreshed 2026-05-10; awesome-monorepo §9 added 2026-05-15; native PM baseline §1.0 added 2026-05-18)
Subject: `@visulima/vis` v1.0.0-alpha.17
Compared against: **native PM workspace scripts (`pnpm -r`, `bun --filter`, `npm -ws`, `deno task`, `yarn foreach`) — the real adoption baseline, §1.0**, then Nx 21, Turborepo 2.6, moon 2.0 "Phobos", Vite+ alpha + Vite Task (VoidZero, Mar 13 2026), Lage, Rush, Lerna 9, Bazel/Buck2/Pants/Please, Bit, plus point-tools (Knip, syncpack, sherif, manypkg, Wireit, Preconstruct). Section 9 cross-references the [korfuri/awesome-monorepo](https://github.com/korfuri/awesome-monorepo) curated list and adds gaps not surfaced from competitor-issue mining.

---

## Section 1 — Competitor feature inventory

### 1.0 Native package-manager workspace scripts — the real baseline (added 2026-05-18)

Every entry below (§1.1–§1.10) is another orchestrator. But the actual default that ~80% of monorepo teams start from — and the thing vis must beat to justify _any_ adoption — is the package manager's own workspace script runner. Bun and Deno appear elsewhere in this doc only as toolchains; here they are evaluated **as task runners**, which is how they actually compete at the low end (the Ultra Runner / Lage / "I just want `build` to run everywhere" audience).

**The four baselines:**

- **`pnpm -r run <script>` / `pnpm --filter`** — the strongest native baseline by far. `-r` runs the script in every workspace package **in topological (dependency) order** with `--workspace-concurrency`. `--filter` is a real selector language: name globs, `pkg...`/`...pkg` (dependents/dependencies), `{dir}` path filters, and crucially **`--filter "...[origin/main]"` = git-based affected detection out of the box**. This is the single most important fact in this section: pnpm already ships topological ordering _and_ changed-since filtering for free. What it lacks: no task result caching (every run is cold), no task-level inputs/outputs (affected is package-granular, not "did the inputs to _this task_ change"), no cross-package task graph beyond install-dependency order (can't express "`test` depends on sibling `build`"), no watch coordination, no remote cache.
- **`bun --filter '*' run <script>`** (Bun ≥1.1) — zero-config, near-instant startup, clean prefixed interleaved output, parallel by default. Growing fast at the low end. Lacks: topological ordering guarantees, caching, affected detection, task graph, watch orchestration. It is the "fast and dumb" baseline — and "fast" is doing a lot of adoption work.
- **`npm run <script> --workspaces` (`-ws`)** — runs the script across all workspaces, **sequentially, in package.json order (no dependency-graph awareness)**. `--include-workspace-root`, `--workspace=<name>` for scoping. No caching, no affected, no parallelism, no ordering. The weakest baseline, but the default for the largest install base.
- **`deno task` + Deno 2 workspaces** — `deno task` runs across workspace members; Deno caches _dependency compilation_ but **not task results**. No affected, no task graph orchestration, no watch coordination beyond `deno task --watch` per-process. Small but growing monorepo audience.

(Honorable mention: **`yarn workspaces foreach -pt run <script>`** via Yarn Berry's `plugin-workspace-tools` — `-p` parallel, `-t` topological, `-R` recursive-from-focus. This is closer to a _real_ runner than the other three natives and the most defensible "I don't need vis" position on Yarn repos. Still: no result caching, no task-level inputs, no affected beyond `--since`.)

**Capability vs the native baseline:**

| Capability                         | pnpm -r / --filter | bun --filter | npm -ws | deno task | yarn foreach | **vis** |
| ---------------------------------- | ------------------ | ------------ | ------- | --------- | ------------ | ------- |
| Topological ordering               | ✓                  | —            | —       | —         | ✓ `-t`       | ✓       |
| Changed-since / affected           | ✓ `[ref]` filter   | —            | —       | —         | ~ `--since`  | ✓       |
| Parallel execution                 | ✓ concurrency      | ✓ default    | —       | —         | ✓ `-p`       | ✓       |
| Task **result** caching            | —                  | —            | —       | —         | —            | ✓       |
| Task-level inputs/outputs hashing  | —                  | —            | —       | —         | —            | ✓       |
| Cross-package task graph (non-dep) | —                  | —            | —       | —         | —            | ✓       |
| Remote cache (CI reuse)            | —                  | —            | —       | —         | —            | ✓       |
| Watch coordination across packages | —                  | —            | —       | —         | —            | ✓       |
| Selector / query language          | ✓ filter           | ~ glob       | ~ name  | —         | ~ from       | ✓ MQL   |

**The "why upgrade from `pnpm -r`" wedge — the most common adoption conversation, and it's the one the rest of this doc never has.** Against Nx/Turbo the pitch is feature breadth; against `pnpm -r` that pitch is irrelevant — the team already has topological order and git-affected filtering for free and feels no pain. The honest marginal value vis adds over `pnpm -r run build` is exactly three things, and the pitch must lead with them, not with the §3 feature wall:

1. **Caching** — `pnpm -r` is always cold; the second `build` re-runs everything. vis skips unchanged packages. This is the entire value proposition for a team that has never felt a slow CI yet — and it only lands once the repo is big enough to hurt, so the wedge is "adopt before it hurts, pay zero cost now."
2. **Task-level (not package-level) affected + input hashing** — `pnpm --filter "...[main]"` rebuilds a whole package if _any_ file changed; vis rebuilds only tasks whose declared inputs changed (README edit ≠ rebuild).
3. **A task graph that isn't the install-dependency graph** — `pnpm -r` can only order by `dependencies`/`devDependencies`. "`e2e` depends on sibling `build` + a root `db:migrate`" is inexpressible natively and trivial in vis.

**Verified implementation status (code-checked 2026-05-18, `vis` alpha):** the migration-cost-zero requirement is **already met for target discovery and ordering, but not for cache correctness** — a sharper finding than "gated on inferred tasks":

- **Target discovery is zero-config today.** `createTargetsFromScripts(pkg.scripts, …)` runs unconditionally for every workspace package (`src/config/workspace.ts:748`); packages without a `project.json` are _not_ skipped (only packages with no `name` are, `:728`). So `vis run :build` already resolves on a bare pnpm/npm/bun monorepo with nothing but `package.json` scripts. The `inferTargets` flag (`:757`, default off) gates only the _bonus_ tool-config detectors, **not** the baseline script-wrapping.
- **Dependency ordering is zero-config today.** `buildProjectGraph` (`:825–872`) derives the graph purely from `package.json` `dependencies`/`devDependencies`/`peerDependencies` that resolve to workspace projects. This is exactly `pnpm -r`'s install-dependency ordering — **parity, for free, no config**. (Wedge value-prop #3 — a task graph richer than the install graph — is the _upgrade_, not the floor, and it correctly still needs declared `dependsOn`.)
- **The cache-correctness gap is `outputs`, not `inputs`.** A missing `inputs` is _coarse-but-safe_: the hasher falls back to `{projectRoot}/**/*` (`task-runner/src/task-hasher.ts:497–499`), so it over-invalidates (a README edit re-runs the build) but **never serves a stale hit**. The real hazard is a _cacheable `build` with no `outputs`_: `resolveOutputs` returns `[]` (`output-resolver.ts:50–52`), so a cache hit restores **nothing** and a downstream consumer sees a missing `dist/`. The earlier "no inputs = stale/unsafe" framing was wrong; the precise gate is outputs on build targets.

**Implemented 2026-05-18** (this is now partly shipped, not just analysis):

1. `inferTargets` now defaults **on** (`resolveInferTargetOption`, `workspace.ts`; schema/types `@default true`).
2. **Guarded enrichment** — when a `package.json` script owns a target name and its command _is_ a detector's command (optionally with extra flags, no shell chaining; `scriptMatchesInferredCommand`), the script keeps its command but adopts the detector's precise `inputs`/`outputs` and `type`. A detector audit confirmed every common build detector (vite, tsup, tsdown, packem, rollup, rolldown, webpack, next, nuxt, remix, astro, typescript) declares `outputs`, so `"build": "vite build"` now caches precisely and restores artifacts with **zero config**.
3. A script that _matches a detector by name but not command_ (compound/custom, e.g. `vite build && tsc`) adopts the detector's `type` (so it's recognised as a build) but **not** the precise outputs; a `build` + cacheable + no-outputs target is then forced `cache:false` — _correct-but-cold_ rather than silently wrong. A script with **no** matching detector stays uncached (no footgun caching `dev`/`clean`/`release`).

**Remaining gap (the one real piece of new work):** compound/custom build scripts are cold. Closing that needs **auto-write output capture** (`outputs: [{ auto: true }]`) wired through the task-runner config→graph path — today `TargetConfiguration.outputs` is typed `string[]` and `task-graph.ts:67` string-manipulates it, so `{ auto: true }` is only understood in the cache/`output-resolver` layer, not declarable as a config default. That is a contained **task-runner** change (widen `outputs` to `OutputSpec[]` + fix the ~handful of string-assuming consumers), not a vis-config or "build inferred tasks" effort — see updated §5.

Sources: pnpm.io/cli/recursive, pnpm.io/filtering, bun.com/docs/cli/filter, docs.npmjs.com/cli/v10/using-npm/workspaces, docs.deno.com/runtime/fundamentals/workspaces, yarnpkg.com/cli/workspaces/foreach.
...
| 10  | **Source-mapped, code-framed task-failure rendering**        | nobody else in JS-land (structural) | vis owns `@visulima/error` in-monorepo (`parseStacktrace` + `@visulima/source-map` + `codeFrame`); competitors pass task stderr raw and can't copy without an error-rendering library. Free, native, currently unused (zero imports in vis/task-runner). Compounds `vis ai heal` input quality. See §8.7 / roadmap #14. | Medium                           |
...
**Reframing after §1.0 (corrected by code-check, see §1.0 "Verified implementation status"):** the adoption funnel doesn't start at "Nx vs vis" — it starts at "`pnpm -r` vs anything." The doc previously had no answer to "why upgrade from `pnpm -r run build`." Code inspection shows the answer is in better shape than first thought: **zero-config target discovery and pnpm-parity dependency ordering already ship today** (`workspace.ts:748`, `:825`), and as of 2026-05-18 inference is **on by default** with guarded detector enrichment. The remaining hazard was never no-`inputs` (the hasher already falls back to `{projectRoot}/**/*` — coarse but never stale, `task-hasher.ts:497`); it is a cached _build_ target with no `outputs`, which restores nothing on a hit (`output-resolver.ts:50`). That last piece is a contained task-runner change, not a feature build.

If picking 3–4 items for the next milestone, the strongest hand is:

1. **Zero-config safe caching — the adoption lever.** **Shipped 2026-05-18:** (a) `inferTargets` defaults on so the existing tool-config detectors supply `inputs`/`outputs` (`workspace.ts` `resolveInferTargetOption`, `config/types.ts` `@default true`); (b) detector input/output coverage audited — all common build detectors (vite, tsup, tsdown, packem, rollup, rolldown, webpack, next, nuxt, remix, astro, typescript) declare `outputs`; (c) a `package.json` script keeps its command verbatim but adopts the detector's `inputs`/`outputs` only when the command matches the detector base (no shell chaining) — guarded enrichment; (d) a compound build script (`vite build && tsc`) is forced `cache: false` (correct-but-cold, not wrongly-cached); (e) a script with no detector match is left uncached (no footgun). **Remaining gap — the real next piece:** auto-write output capture so even an unrecognised build script caches correctly without hand-written `outputs`. This is a contained task-runner change, not a vis config default: `TargetConfiguration.outputs` is `string[]` and `task-graph.ts:67` string-manipulates it, so the `{auto:true}` `OutputSpec` (already present in the cache layer) must be threaded through the config→graph path. Ship a one-command quickstart: "point vis at your pnpm monorepo, `vis run :build`, second run is cached." Verify end-to-end on a fresh fixture repo with only `package.json` scripts before claiming the wedge.
2. **MCP server** — low effort, high signal in the AI-tooling year. Nx alone owns this in JS-land.
3. **Self-healing CI loop** on top of `vis ai` — Nx is the only competitor with this; you already have AI primitives.
4. **REAPI gRPC cache backend** — already on `todo.md`. Single big-ticket cache differentiator vs Turbo/Nx/Lage. Pair with the existing HTTP backend, don't replace it.
...
- Native PM baseline (§1.0): pnpm.io/cli/recursive, pnpm.io/filtering, bun.com/docs/cli/filter, docs.npmjs.com/cli/v10/using-npm/workspaces, docs.deno.com/runtime/fundamentals/workspaces, yarnpkg.com/cli/workspaces/foreach
...
Second-order insight (the gap behind both directions): inbound migrators and `vis export` are **coverage**, not **correctness** — neither side proves the converted config yields the _same task graph / same cache-key surface / same affected closure_. "Adopt risk-free" / "leave any time" stays a leap of faith until equivalence is verifiable. The missing command is `vis migrate verify` (inbound: diff source-parsed graph vs migrated graph) + `vis export --verify` (outbound: `import ∘ export` round-trip fixpoint, no foreign tool install required; optional `--cross-check` against the real tool's dry-run). This is the migration analogue of cache restoration fidelity (§8.1 Theme O / roadmap #11): coverage without fidelity is unproven. Shipped as roadmap #13.
...
16. **Migration equivalence verification** (`vis migrate verify` / `vis export --verify`) — proves the converted config preserves the task graph / cache-key surface / affected closure. The correctness half of #9 and the inbound §3 migrators; no JS-land competitor has it (Nx Console ships diff preview only). Roadmap #13.
...
### 8.6 — Attested provenance & keyless artifact signing (added 2026-05-17; s1ngularity lead added 2026-05-18)

Not issue-mined — but no longer hypothetical. The 2026 supply-chain compliance landscape supplies the _standards_ argument; the **August 2025 s1ngularity attack on Nx** supplies the concrete, high-salience _incident_ argument. This is the largest unclaimed differentiator adjacent to vis's existing security surface — and the one place in this entire analysis where "the most popular tool in this category shipped credential-stealing malware to its users" is a literal, citable fact rather than a projected risk. The earlier framing here ("no competitor ships this, so no issues are filed against it; the absence _is_ the signal") still holds for the _outbound_ primitives — but it understated the case. s1ngularity is the signal that is _not_ an absence.

**The defining incident — s1ngularity (Nx, 26 Aug 2025).** Attackers exploited a `pull_request_target` code-injection flaw in an Nx GitHub Actions workflow to exfiltrate the npm publish token, then published malicious `nx` + `@nx/*` / `@nrwl/*` versions for a ~4-hour window. The rogue versions carried a **postinstall** script (`telemetry.js`) that: scanned the filesystem for credentials (npm/GitHub tokens, SSH keys, `.env`, crypto wallets); **weaponised locally installed AI CLIs (Claude, Gemini, `q`) to enumerate sensitive files** — the first known supply-chain attack to turn developer AI assistants into recon tooling; base64-exfiltrated the loot into a public `s1ngularity-repository` created on each _victim's own_ GitHub account; and appended `sudo shutdown -h 0` to `.zshrc`/`.bashrc`. A second wave hit 190+ orgs, 3000+ repos, 2,349 distinct leaked secrets (GitHub OAuth/PATs dominant, then AWS / Anthropic / OpenAI / Google AI / Postgres / Datadog). Nx's remediation was to adopt **npm Trusted Publishers (OIDC) + manual release approval** — i.e. the exact provenance/keyless direction this section argues for, retrofitted under fire.

**Why this is vis's single strongest security wedge — the failure modes map 1:1 onto already-shipped defenses:**

- **No postinstall.** vis (and `@visulima/task-runner`) ship zero install-time scripts; the entire attack payload was a postinstall hook. This is a static, auditable property an adopter can verify in seconds — not a roadmap promise.
- **Inbound provenance regression detection (§3, shipped).** `src/security/marshalls/provenance.ts` flags exactly the "a prior version had `dist.attestations.provenance`, this resolved version dropped it" pattern — the literal fingerprint of a compromised-CI-key publish. s1ngularity is the worked example the marshall was built for.
- **Inbound registry-signature verification (§3, shipped).** `signatures.ts` ECDSA-verifies `dist.signatures` against npm's keys — catches the unsigned/forged-publish vector.
- **Outbound attestation (roadmap #12, shipped).** `vis attest` + keyless-signed cache mean a vis-built artifact carries SLSA provenance the s1ngularity packages never could.
- **AI surface kept read-only by intent.** The attack's novel rung was hijacking AI CLIs for recon. vis's MCP server (item 5) is deliberately read-only — an LLM client cannot mutate the workspace or shell out through the protocol — so the same AI surface that amplified s1ngularity is, in vis, structurally inert.

**The pitch, now backed by an incident not an inference:** _the most-installed tool in this category shipped credential-stealing malware via postinstall to thousands of repos; vis has no postinstall, verifies the provenance and signatures of what it consumes, and attests what it produces._ Note the irony worth deploying in positioning: Nx's own fix (OIDC Trusted Publishers) is the same primitive Lerna 9 added (§1.7, also Nx-owned) and that this roadmap tracks — the category is converging on vis's thesis reactively, after a breach, rather than by design.
...
**Sources:** s1ngularity — nx.dev/blog/s1ngularity-postmortem, thehackernews.com/2025/08/malicious-nx-packages-in-s1ngularity.html, wiz.io (second-wave 190+ orgs / 2,349 secrets), stepsecurity.io/blog/supply-chain-security-alert-popular-nx-build-system-package-compromised-with-data-stealing-malware, infoq.com/news/2025/10/npm-s1ngularity-shai-hulud (AI-CLI weaponisation). Provenance primitives — docs.npmjs.com/generating-provenance-statements, slsa.dev/spec/v1.0, github.com/actions/attest-build-provenance, docs.sigstore.dev/cosign/signing/signing_with_blobs, turborepo.dev/docs/reference/configuration#signature (HMAC scope), §1.2 / §1.7 / §3 / §8.4 #28 cross-refs.

### 8.7 — Source-mapped, code-framed task-failure rendering (the `@visulima/error` cross-package moat) (added 2026-05-18)

Not issue-mined — the same "absence is the signal" class as §8.6's outbound primitives, and structurally stronger. This analysis enumerates 30+ borrowable features across §1–§9 but never the one differentiator that is free, native, and structurally impossible for the competition to copy: **vis can render task failures with source-mapped, syntax-highlighted, code-framed stack traces because it lives in a monorepo whose flagship brand is error-debugging — and no competitor owns an error-rendering library.**

**The capability already exists in-house, unused.** `@visulima/error` (`packages/error-debugging/error/`, v6.0.0-alpha.25): `parseStacktrace` (V8/Firefox/Safari/Node/React-Native dialects), `codeFrame` (syntax-highlighted, configurable context lines), `renderError`, `getErrorCauses`, `serializeError`/`deserializeError`; source-map resolution via `@visulima/source-map` (3.0.0-alpha.9); CLI rendering already packaged as `@visulima/error-handler/handler/cli` (2.0.0-alpha.24). **Neither `@visulima/vis` nor `@visulima/task-runner` depends on it — zero imports.** vis's failure output today is raw PTY passthrough: `src/tui/components/task-store.ts:168-169` (256 KB buffer, `:190`), rendered unprocessed in `output-panel.tsx:99-145`, summarised as bare task-IDs in `command-summary.tsx:114-131`, raw fetch in `vis-task-runner-app.tsx:215`.

**Why this is copy-proof, not just unbuilt.** Turbo, Nx, moon, Lage, Rush, and the Bazel family all pass task stderr through raw — none ships a stack-trace parser + source-map resolver + syntax-highlighting code-framer, because that is an error-rendering product, orthogonal to a task runner, and not their surface. For a competitor it is a multi-month build; for vis it is a workspace import. This is the §8.6 pattern (a wedge that exists because of what the surrounding monorepo _is_) applied to the failure-rendering surface — and unlike §8.6 it requires no external dependency, no CI OIDC, no transparency log: the lowest-cost structural moat in the analysis.

**It compounds the AI moat (§4 Tier-1 #4 / roadmap #8).** A parsed, source-mapped, code-framed failure is a materially better `vis ai heal` model input than raw stderr — better heal proposals, and an input-quality edge competitors can't match without the same library. The moat stacks onto a shipped flagship rather than competing with it.

**Net positioning.** "vis renders _why your build failed_ the way `@visulima/pretty-error` renders an exception — source-mapped, syntax-highlighted, code-framed — because vis is the only task runner whose vendor also ships the error-debugging stack. Every other runner shows you the same raw stderr your shell already did." Tracked as **roadmap #14** (ranked above Tier B by leverage; demand ★ novel, same profile as #12 pre-incident).

**Sources:** Internal — `packages/error-debugging/error/`, `@visulima/source-map`, `@visulima/error-handler/handler/cli`; vis failure-render path `src/tui/components/{task-store,output-panel,command-summary}` + `vis-task-runner-app.tsx`. Cross-refs §3 (baseline error-debugging ownership), §8.6 (analogous structural-moat shape), §4 Tier-1 (new row #10).
