...
```typescript
function transformDMMF(dmmf: DMMF.Document, options?: TransformOptions): JSONSchema7;
```
...
```typescript
function getJSONSchemaProperty(field: DMMF.Field, options?: TransformOptions): JSONSchema7Definition;
```
...
```typescript
type PrismaPrimitive = "String" | "Boolean" | "Int" | "BigInt" | "Float" | "Decimal" | "DateTime" | "Json" | "Bytes";
```
