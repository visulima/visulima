...
**Yarn Berry integrity is not supported.** Berry records `checksum: 10c0/…` (XXH64), which is not a cryptographic hash and is outside the CycloneDX 1.6 `HashAlgorithm` enum. Berry entries come out of the parser with `integrity: undefined`. Callers that need Berry integrity must read `yarn.lock` directly.
