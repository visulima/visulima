 ...
 *
 * Ordering rationale:
 * - `pnpm-lock.yaml` first: after a migration a stale `yarn.lock` is often left behind, so the pnpm lockfile should win when both are present (this matches the parser module).
 * - `npm-shrinkwrap.json` shares `package-lock.json`'s JSON shape and is grouped with it.
 * - `bun.lock` (modern text) before `bun.lockb` (legacy binary) so a migrated project with a stale binary lockfile still picks the parseable one.
 */
