...
| CI packaging of helper bin per platform                                         | 0.5d   | ✅ Done — `build-native.yml` now builds the helper for the four Linux triples via `cargo zigbuild`, installs it into `npm/linux-*/`, and the binding packages' `files` arrays include it. Cache key bumped to hash `fspy_seccomp/**`. Needs one real CI cycle to validate.                                 |
| musl + Alpine test (CI matrix addition)                                         | 1d     | ✅ Done — new `.github/workflows/fspy-seccomp-alpine.yml` runs `cargo test` for the crate inside `node:24-alpine`. Verifies musl + Alpine's libseccomp + container `CAP_SYS_PTRACE`. Needs `--privileged` container option since GitHub Actions doesn't expose `--cap-add`.                                |
| Performance benchmark                                                           | 0.5d   | ✅ Done — `tests/perf.rs` (`cargo test --test perf --release -- --ignored`). On this devbox: 500-file shell pipeline = 7.5ms untracked → 17.9ms tracked (+10ms, ~2.4× wall-clock); 1351 access events captured. Negligible vs build-tool seconds. vs-strace comparison deferred (strace not on this host). |
