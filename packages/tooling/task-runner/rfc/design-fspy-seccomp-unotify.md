...
| musl + Alpine test (CI matrix addition)                                         | 1d     | ✅ Done — new `.github/workflows/fspy-seccomp-alpine.yml` runs `cargo test` for the crate inside `node:24-alpine`. Verifies musl + Alpine's libseccomp + container `CAP_SYS_PTRACE`. Needs `--privileged` container option since GHA doesn't expose `--cap-add`.                                           |
