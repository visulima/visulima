...
- **`fork`/`clone` of children.** The seccomp filter is inherited across `fork`/`clone`, but the kernel installs a _separate_ notify fd per new task — our supervisor only holds the root's. Two viable patterns; pick one:
    1. **Discovery via `/proc`.** Poll `/proc/<root>/task/` and `/proc/<root>/children` on a short tick (≈50 ms). For each new pid, `PTRACE_SEIZE` to grab the existing notify fd, register it with our epoll loop. Cheap but has a discovery race for very-short-lived grandchildren.
    2. **Discovery via `clone3` notifications.** Add `clone` / `clone3` to the seccomp filter as user-notif; on each event, capture the new pid from the result and immediately attach. No race, but pays one notification per fork — measurable overhead on fork-heavy workloads (shells, make, autotools).
       Default to (1) for the v0 — most build tools fork rarely. Document the limitation and the (2) escape hatch behind a `VIS_SECCOMP_FORK_TRACK=precise` env knob.
