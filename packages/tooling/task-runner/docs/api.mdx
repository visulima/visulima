...
if (isNativeAvailable()) {
    ...
    const main = native?.getMainWorktreeRoot(workspaceRoot);
}
```

### Worktree detection

When the addon is loaded, the bindings expose three worktree helpers used by affected detection and lockfile resolution to point at the right `.git` directory inside linked git worktrees:
...
Pure-TypeScript fallbacks ship for everything else the native addon exposes (parallel file hashing, cycle detection, concurrent process runner) so callers never need to branch on platform.
