...
- **Baseline JSON** (the sole fingerprint store) with merge + diff, inline (`gitleaks:allow`) and block (`gitleaks:allow-start` / `-end`) suppression markers. Also accepts `secret-scanner:allow`.
...
// Compute a gitleaks-compatible fingerprint (`file:ruleID:startLine`)
