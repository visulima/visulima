...
> **Phase 1 + phase 2 (PRs #1–#6) landed.** All **825 / 825** Kingfisher rules are imported. Native detector probe-compiles each rule into `regex::bytes::Regex` (Fast) when possible, falling back to `fancy_regex` (Fancy); Fast rules participate in a global `regex::bytes::RegexSet` prefilter (64 MB DFA budget). Each rule's AC keyword hits seed a **lookback-window** (±4 KiB) so per-rule `captures_iter` only runs over the slice around its own keyword, not the whole file. Compiled rulesets are cached in-process. Baseline fingerprints are content-addressed (sha256 over `secret + ruleId + file`, truncated to 16 hex) and survive line shifts; legacy `file:rule:line` baselines still suppress. **Default ruleset is `"combined"` (1,047 rules active by default; 1,058 total when the 11 opt-in preset rules are enabled).**
>
> The opt-in validator (`config.validate: true` / `--validate`) handles:
>
> - **HTTP** — `type: Http` with `StatusMatch`, `WordMatch`, `JsonValid`, and `HeaderMatch` response matchers against a liquid-lite template (`{{ TOKEN }}` + dep variables; filters `downcase`/`upcase`/`b64enc`/`b64dec`).
> - **`depends_on_rule`** — when a rule declares a dependency, the validator finds a matching dep-rule finding in the same file and injects its secret as the named variable (AWS AKID+secret pairing, OAuth id+secret, 106 rules total).
> - **Checksum** — TS-side CRC32 filter drops findings whose embedded checksum doesn't match the body. Runs _before_ HTTP validation. 11 rules (Zuplo, Vercel, npm, GitHub, Confluent, …).
> - **JWT** — offline formal-validity check (3 base64url segments, JSON-parsable header + payload, `alg` field present). No network.
> - **Per-host rate limiting** — layered under the global 8-wide concurrency cap; honours `Retry-After` on 429/503, pauses the offending host for subsequent calls.
>
> `HttpMultiStep` validation types don't exist in the upstream YAML (the 13 HttpMultiStep blocks all live under `revocation:` — destructive, deliberately out of scope). Non-HTTP transports (AWS/GCP/Mongo/Postgres/MySQL/Jdbc/Grpc/Raw) mark findings as `validation: "skipped"`; each would add one npm dep per ≤2 rules, so they belong in external plugins rather than core.
>
> `--only-verified` filters to verified findings for CI gating. Measured: **~11 ms per 550 KB** scan with the full 1,047-rule set. See `scripts/kingfisher.ref` for the pinned upstream commit + `data/kingfisher.skipped.log` (empty — every rule imports).
...
- Verified counts from source: Kingfisher v1.95.0 ships **825 rules** (counted on imported `data/kingfisher.json`, `source == "kingfisher"`), of which **504 declare HTTP validators**, 11 use native validators (1 AWS + 1 GCP + 2 MongoDB + 1 Postgres + 1 MySQL + 1 Jdbc + 1 JWT + 1 AzureStorage + 2 Coinbase), 1 uses gRPC, and 5 `Raw` validators remain unclassified — 521 / 825 have some validator. HttpMultiStep only appears in upstream `revocation:` blocks (13 occurrences) — deliberately out of scope, not imported. Our gitleaks portion is 222 upstream rules, no validators.
...
- We are **ahead** on: Node.js embeddability (NAPI, no shell-out), gitleaks ruleset parity + fingerprint compatibility, minimal footprint (7 Rust source files, ~1,891 LOC; Kingfisher `src/` alone is ~11k LOC with 40+ provider modules), `vis` ergonomics (workspace-aware `--affected`/`--staged`/`--since`, c12 config across TOML/JSON/YAML/TS/JS), and deterministic sorted output.
...
- Confidence filter on load (`get_builtin_rules(Some(Confidence::Medium))` is the default — `defaults.rs:43`). Observed distribution across 825 rules: **647 medium, 94 high, 32 low, 52 unlabeled** (`jq '.rules | map(select(.source == "kingfisher")) | map(.confidence) | group_by(.)'` over `data/kingfisher.json`).
- Rule fields observed in `/tmp/kingfisher/crates/kingfisher-rules/data/rules/aws.yml` and `stripe.yml`:
    ...
    - `pattern_requirements`: `{ min_digits, min_uppercase, min_lowercase, ignore_if_contains: [...] }` — **642/825 rules use this**. Provides fast structural rejection before entropy check. We have no equivalent.
    - `min_entropy`: float Shannon threshold — **811/825 rules set it** (virtually universal).
    - `depends_on_rule`: chains rules by ID and exports captured groups as **liquid template variables** (e.g. AWS secret-key rule depends on AKID rule with `variable: AKID`) — **106 rules use cross-rule dependencies**. Unique feature.
    - `validation`: `{ type: Http | HttpMultiStep | AWS | GCP | MongoDB | MySQL | Postgres | Jdbc | JWT | AzureStorage | Coinbase | Grpc, content: {...} }`. **521 of 825 rules have validators** (504 HTTP + 11 native + 1 gRPC + 5 Raw).
...
- **Ruleset:** Gitleaks `gitleaks.json` (MIT) at `packages/tooling/secret-scanner/data/gitleaks.json` — fetched from a pinned upstream ref (`scripts/gitleaks.ref`) and merged with `data/gitleaks.patches.json` at build time via `scripts/build-rules.mjs`. Plus tagged opt-in rule groups (`weak-passwords`, `password-manager`) merged into every ruleset but disabled-by-default; enable with `rules.include: ["tag:preset:<name>"]`. Runtime is JSON-only — no c12/TOML parser in the dep graph (`src/config-loader.ts`).
- **Walker:** `ignore` 0.4 crate `WalkBuilder::build_parallel` with `.gitignore` / `.ignore` / `.git/info/exclude` / global gitignore, plus `walk.excludePatterns` + `walk.excludeFromFiles` (public names; native layer exposes them as `extra_ignores` + `ignore_files` in `native/src/walker.rs`).
...
- **Rust footprint:** 7 source files, ~1,891 LOC total (`wc -l native/src/*.rs`).
...
| ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| Rule count                                                   | **825** (upstream v1.95.0, imported 1:1 into `data/kingfisher.json`)                                                                                                                | 222 (gitleaks JSON) + 11 opt-in presets                                                                 | Large; we inherit gitleaks. Importing extra rulesets is cheap — format is gitleaks-shaped.                                 |
...
| Regex engine                                                 | **`vectorscan-rs` 0.0.6 (BSD vectorscan)** + per-rule `regex::bytes::Regex` for span recovery                                                                                       | `fancy-regex` per rule + `aho-corasick` prefilter                                                       | They scan all 825 patterns in a single SIMD pass. We AC-prefilter then run each rule's regex. Big gap on text-heavy files. |
...
| **Pattern structural requirements**                          | `min_digits`, `min_uppercase`, `min_lowercase`, `ignore_if_contains` on 642/825 rules                                                                                               | **None**                                                                                                | Medium gap; fast FP rejection without extra regex.                                                                         |
| **Rule dependencies**                                        | `depends_on_rule` + liquid vars; 106 rules (e.g. AWS secret needs AKID)                                                                                                             | None                                                                                                    | Unique; enables multi-token secrets (AKID+secret, OAuth id+secret).                                                        |
...
| **Live validation**                                          | 504 HTTP + 11 native + 1 gRPC + 5 Raw validators (+ 13 HttpMultiStep in upstream `revocation:` only, not imported); `--only-valid`, `validate` subcmd, per-rule RPS, response cache | **No**                                                                                                  | Largest feature gap.                                                                                                       |
...
1. **Vectorscan SIMD multi-pattern scan.** A single pass matches all 825 patterns using AVX2/SSE4.2 literal acceleration (`rules_database.rs:28`, `matcher/mod.rs`). Our `aho-corasick` keyword DFA narrows to candidate rules, but each candidate then runs a full `fancy_regex` execution independently. Expect **2–5× on text-heavy files** with many concurrently-applicable patterns.
...
3. **`pattern_requirements` fast path.** Digit/case-count checks reject candidates before entropy calc or secondary regex (`src/matcher/filter.rs:~60+`). Saves Shannon-entropy work on the 642/825 rules that declare them.
...
12. **Rule pack plugins.** Publish `@visulima/secret-scanner-rules-ai`, `-cloud-extra`, etc., to narrow the rule-count gap without bloating default install. Kingfisher's 825-rule default install bundles 40+ provider SDKs; we can stay lean by shipping packs opt-in.
...
- Kingfisher rules (825 at v1.95.0, counted on imported `data/kingfisher.json` with `source == "kingfisher"`): `/tmp/kingfisher/crates/kingfisher-rules/data/rules/`
