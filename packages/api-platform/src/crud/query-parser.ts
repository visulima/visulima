...
import type { IParsedQueryParams, TOrderByField, TRecursiveField, TWhereField } from "./types";
...
const parseOrderBy = (orderBy: string): TOrderByField => {
    ...
    if (Object.keys(orderByObj).length >= 1) {
        ...
        if (orderByObj[key as keyof typeof orderByObj] === "$asc" || orderByObj[key as keyof typeof orderByObj] === "$desc") {
            parsed[key as string] = orderByObj[key as keyof typeof orderByObj];
        }
    }
