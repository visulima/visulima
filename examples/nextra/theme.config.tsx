...
const config: DocumentationThemeConfig = {
    ...
    editLink: {
        content() {
            ...
            return <>{EDIT_TEXT[locale as keyof typeof EDIT_TEXT]}</>;
        },
    },
    feedback: {
        ...
        labels: "feedback",
        link(title) {
            return `https://google.com/search?q=${encodeURIComponent(`Feedback for ${title}`)}`;
        },
    },
    footer: {
        copyright() {
            ...
            return (
                <a rel="noopener" target="_blank" className="flex items-center gap-2 font-semibold" href={FOOTER_LINK[locale]}>
    ...
    i18n: [
        ...
        { direction: "rtl", locale: "es-ES", name: "Español RTL" },
    ...
    logo() {
        ...
        return (
            <>
                ...
                <span className="hidden select-none font-extrabold ltr:ml-2 rtl:mr-2 md:inline" title={`SWR: ${TITLE[locale] || ""}`}>
                    SWR
                </span>
            </>
        );
