...
<Callout>请注意，`initialSize` 选项不允许在运行时更改。</Callout>
...
<Callout emoji="✅">请升级至最新版本（≥ 2.1.0）以使用此 API</Callout>
...
<Callout emoji="⚠️">当你启用了 `parallel` 选项，`getKey` 函数的参数 `previousPageData` 会变为 `null`。</Callout>
...
<Callout emoji="⚠️">正如命名所示，`unstable_serialize` 目前并不是一个稳定的 API，所以我们可能会在未来修改它。</Callout>
