...
<Callout emoji="⚠️">
    Using global mutator only with the `key` parameter will ***not update the cache or trigger revalidation*** unless there is a mounted SWR hook using the same
    key.
</Callout>
...
<Callout>
    Распространяется на SWR хуки в одной области [провайдера кеша](/docs/advanced/cache). Если провайдера кеша не существует, будет распространяться на все SWR
    хуки.
</Callout>
...
mutate((key) => typeof key === "string" && key.startsWith("/api/item?id="), undefined, { revalidate: true });
```
