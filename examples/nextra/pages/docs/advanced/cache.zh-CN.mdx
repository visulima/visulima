...
<Callout>升级到最新版本（≥ 1.0.0）来使用该功能。</Callout>
...
<Callout emoji="🧪">这是一个实验性的功能，在未来的升级中可能会发生变化。</Callout>
...
function Button() {
    ...
    return <button onClick={() => matchMutate(/^\/api\//)}>重新请求所有以 `/api/` 开头的 key</button>;
}
```

<Callout>注意，该示例要求缓存 provider 为 Map 实例。</Callout>
...
<Callout>作为一种改进，你还可以使用内存缓存作为缓冲区，并定期写入 `localStorage`。你还可以使用 IndexedDB 或 WebSQL 实现类似的分层缓存。</Callout>
