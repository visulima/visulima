...
<Callout>Atualize para a última versão (≥ 1.0.0) para usar este recurso.</Callout>
...
<Callout>
    No exemplo acima, quando o componente `<App />` é re-montado, o provider também é re-criado. Cache Providers devem ser colocados mais alto na árvore de
    componentes, ou fora do render.
</Callout>
...
<Callout emoji="🧪">Este é um recurso experimental, o comportamento pode mudar em futuras atualizações.</Callout>
...
<Callout>
    Como melhoria, você também pode usar o cache de memória como um buffer e gravar em `localStorage` periodicamente. Você também pode implementar um cache em
    camadas semelhante ao IndexedDB ou WebSQL.
</Callout>
