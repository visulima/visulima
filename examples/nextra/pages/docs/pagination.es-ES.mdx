...
<Callout>Tenga en cuenta que la opción `InitialSize` no puede cambiar en el ciclo de vida.</Callout>
...
<Callout emoji="✅">Please update to the latest version (≥ 2.1.0) to use this API.</Callout>
...
<Callout emoji="⚠️">The `previousPageData` argument of the `getKey` function becomes `null` when you enable the `parallel` option.</Callout>
...
<Callout emoji="⚠️">As the name implies, `unstable_serialize` is not a stable API, so we might change it in the future.</Callout>
