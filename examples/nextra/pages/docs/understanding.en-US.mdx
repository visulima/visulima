...
<Callout emoji="📝">
    Fallback data and previous data are not considered "loaded data," so when you use fallback data or enable the keepPreviousData option, you might have data
    to display.
</Callout>
...
function Stock() {
    ...
    // Otherwise, display the data and a spin that indicates a background
    // revalidation.
    return (
        <>
            <div>${data}</div>
