import { Callout, Tabs, Tab } from "@visulima/nextra-theme-docs/components";
...
<Tabs>
    <Tab title={"pnpm"}>```bash pnpm add swr ```</Tab>
    <Tab title={"npm"}>```bash npm i swr ```</Tab>
    <Tab title={"yarn"}>```bash yarn add swr ```</Tab>
</Tabs>
...
<Callout emoji="💡">
    If you want to use GraphQL API or libs like Axios, you can create your own fetcher function. Check <Link href="/docs/data-fetching">here</Link> for more
    examples.
</Callout>
