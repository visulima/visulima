...
SSR frameworks render the HTML server-side, which means `transformIndexHtml` is not called by Vite and the toolbar script is never injected automatically. Use the `appendTo` option to target a module file that is part of every route instead:
